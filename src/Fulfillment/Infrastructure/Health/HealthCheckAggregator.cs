...
/// <summary><c>specs/shared/openapi.yaml</c>'s <c>HealthResponse</c> schema — <c>{status, checks?}</c>, checks keyed by name, nulls omitted (Contracts.Wire.JsonWire).</summary>
...
/// <summary>design.md §8.2 — pure aggregation over the registered <see cref="IHealthCheck"/> set, separated from <see cref="HealthProbeService"/>'s Kestrel/routing plumbing so <c>HealthCheckAggregationTests</c> drives it directly, over fakes, with no HTTP involved.</summary>
