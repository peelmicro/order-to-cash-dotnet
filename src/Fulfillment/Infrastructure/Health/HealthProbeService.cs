...
/// <summary>
/// design.md §8.1 — a minimal <see cref="WebApplication"/> built INSIDE
/// <see cref="StartAsync"/>, mapping only <c>GET /health/live</c> and
/// <c>GET /health/ready</c>. Needs
...
public sealed class HealthProbeService : IHostedService, IAsyncDisposable
{
    ...
    /// <summary>The port actually bound, read back from Kestrel's own resolved address after <see cref="StartAsync"/> — see the Orders copy's own remarks.</summary>
