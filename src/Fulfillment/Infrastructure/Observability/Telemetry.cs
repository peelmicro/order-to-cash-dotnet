...
namespace OrderToCash.Fulfillment.Infrastructure.Observability;
...
{
    public const string SourceName = "OrderToCash.Fulfillment";
...
{
    public const string MeterName = "OrderToCash.Fulfillment";
    ...
    /// <summary><c>OutboxRelay</c>'s own cycle — this service owns no <c>.dlq</c> topic (design.md §13), so <see cref="OrderToCash.Fulfillment.Infrastructure.Outbox.NoOpDlqDepthGauge"/> always records <c>0</c>.</summary>
    public static readonly Gauge<long> DlqDepth = Meter.CreateGauge<long>("otc_dlq_depth");
}

/// <summary>design.md §9.2 — <c>OTEL_EXPORTER_OTLP_ENDPOINT</c>, read by <c>FulfillmentProgramConfiguration.ConfigureTelemetry</c>.</summary>
...
/// <summary>
/// design.md §5.1 — registered from <c>FulfillmentHost.CreateBuilder</c>
...
{
    public static IServiceCollection AddFulfillmentTelemetry(this IServiceCollection services, Action<TelemetryOptions> configure)
    {
        ...
        services.AddOpenTelemetry()
            .ConfigureResource(r => r.AddService(serviceName: "fulfillment"))
            .WithTracing(t => t
