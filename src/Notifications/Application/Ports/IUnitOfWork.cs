...
/// resolved from the same scoped <c>NotificationsDbContext</c> enlists in the
/// transaction <see cref="ExecuteAsync{T}"/> opens automatically — there is
/// no opaque transaction-context parameter to thread through. Notifications
/// has only one write path (the dedup ledger insert, via the canonical
/// <c>IdempotentConsumer</c> copy) but the port stays byte-for-byte the same
/// shape as every other service's copy so that copy can compose it
/// unmodified.
/// </summary>
