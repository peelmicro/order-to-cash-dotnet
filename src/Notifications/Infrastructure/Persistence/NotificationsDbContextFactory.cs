...
/// update` can build a <see cref="NotificationsDbContext"/> without a
/// running host — the Notifications service has no `Program.cs` yet (that
/// lands with the service's own feature; this feature is schema-only). The
/// connection string mirrors the variable names `.env` already declares
/// (`MSSQL_APP_USER`/`MSSQL_APP_PASSWORD`/`MSSQL_DB_NOTIFICATIONS`/
...
/// cref="NotificationsDbContext"/> through DI when the service is built.
/// </summary>
    ...
    public NotificationsDbContext CreateDbContext(string[] args)
    {
        ...
        var password = Environment.GetEnvironmentVariable("MSSQL_APP_PASSWORD")
            ?? throw new InvalidOperationException(
                ...
                "'dotnet ef' against NotificationsDbContext, e.g.: " +
                "export $(grep -E '^MSSQL_(APP_PASSWORD|APP_USER|DB_NOTIFICATIONS|HOST_PORT)=' .env | xargs)");
