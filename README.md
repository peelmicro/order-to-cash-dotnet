...
| 14 | Health checks, OTel propagation, retry + DLQ — **and the guard-hardening audit** | 🟨 **in progress.** Shipped: retry-then-dead-letter, W3C trace propagation through NATS/Kafka/MS-SQL, structured logs with the correlation id, five metrics, liveness and readiness on all six services, client-idempotent order creation; the operator's cancellation note surviving both compensation branches; and an audit that turned into the phase's real work — an operator cancel racing the saga's own forward progress, a notification sender that degrades instead of retrying a permanent failure into the DLQ, Application→Infrastructure layering guarded and then made able to *see* async-lambda closures, the design-time `dotnet ef` factories guarded, the composition-root delegation guarded, the saga command fast path freed from head-of-line blocking, and the test matrix's stale deferrals closed. Two shared-spec amendments raised and applied to both repositories (`SA-3`, `SA-4`). **The audit is not finished:** it has filed more entries than it has closed, and they are listed in `feature_list.json` |
