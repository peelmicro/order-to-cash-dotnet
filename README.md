...
Every figure below is read from a `progress/history.md` — this repository's own, or #7's, both committed and both cited by phase or feature id so the number can be re-derived rather than trusted. **The honest shape of this data is two eras, not one number.** Early- and mid-project phases (6–14) were consistently *slower* here than in #7, by ratios that cluster around 1.03×–3.1×. Late phases (16 onward, once the web app, the API-test harness and the e2e suite existed to reuse) were consistently *faster*, several with no #7 counterpart to compare against at all because #8's own review process found and fixed defects #7 still carries.

*(This table was corrected at the phase-25 final checkpoint: an Opus review walking `CHECKPOINTS.md` C7 found five of its six original rows citing real figures belonging to the wrong phase — mostly individual phase-10 Billing features' own numbers, mislabeled as phase totals for 6, 8, 9, 10 and 14. Every figure below is re-read directly from each phase's own "closing assessment" table in `progress/history.md`, not carried over from the earlier draft.)*

### Where reuse cost more, not less

| Phase / feature | #8 | #7 | Ratio | Why, in one line |
|---|---|---|---|---|
| Phase 6 (EF Core models) | ≈3 h 36 min total, 3 databases | ≈3 h 30 min total, 3 databases | ≈1.03× — no saving | **A dividend that converged to zero.** A rejection round on the first database ate the whole gain of the other two; #7's third database was itself at its own learning-curve floor, so #8 had nothing left to be faster than |
| Phase 8 (Orders + saga) | ≈19 h 54 min (comparable subset) | ≈11 h 18 min | ≈1.76× slower | The hand-rolled CQRS dispatcher (a licence choice, not a language one) and the saga orchestrator both needed .NET-specific design and proof work #7's ambient package guarantees (`kafkajs`, `@nestjs/cqrs`, MySQL) let it skip |
| Phase 9 (Fulfillment) | ≈5 h 59 min (comparable subset) | ≈4 h 47 min | ≈1.25× slower | Closest to parity of the early code phases — Fulfillment inherited the outbox, relay, dispatcher and responder pattern #8 had already paid for in phase 8 |
| Phase 10 (Billing) | ≈11 h 27 min total | ≈6 h 46 min | ≈1.69× slower | The `.99` simulator and the invoice aggregate, both genuinely new domain logic, not a port; ≈1 h 20 min of that gap is gate-ordered refactoring and backlog closures with no #7 counterpart at all |
...
| Phase 14 (observability + reliability) | ≈24 h 10 min near-continuous agent time | ≈7 h 45 min, approved first time | **≈3.1× — the largest gap in the build** | Four review rounds, three rejected; the review loop alone outlasted #7's entire feature. #7 shipped this first-pass; #8 hand-rolled `IHealthCheck` ports and OTel propagation #7 got from framework packages, and needed three rounds to defend guards that had shipped correct on round one |
| Phase 16 (web app, id 29) | 6 implementer sessions, 4 reviews, 3 rejections, ≈4 h 10 min impl. + ≈2 h 30 min review | 14 implementer passes, 8 reviews (2 rejected), spread over two days | Fewer passes, worse rejection rate | Not like-for-like: #7 built the app incrementally across many small passes, and only 2 of 7 reviews rejected; #8 built the whole app in one pass and then spent 3 of its 4 review rounds, and most of its fix time, turning one prose claim into a guard that cannot be walked past |
...
- **Nothing about writing .NET domain code against a Kafka/NATS/MS-SQL stack was faster than #7's TypeScript equivalent**, in the phases where that was the actual work (6, 8, 9, 10, 14). The specification and harness reuse did not translate into faster *typing* — the ratios above are ≈1.03×–3.1× slower, consistently, for as long as new domain logic was being written for the first time in this stack, and phase 14's ≈3.1× is the single largest gap in the build.
- **A spec pass can be one of the widest gaps on its own feature**, even inside an otherwise-comparable phase: `billing_invoicing` (id 21, phase 10) took a ≈19 min spec pass against #7's own, ≈6× shorter — an EARS requirement written against #7's own vocabulary still needed re-deriving in .NET terms before it was actionable. This is a feature-level number, not a phase-level one; it should not be read as "phase 10's spec gap" or extrapolated to the other phases above.
- **The audit-style phase (14) took more total sessions here**, not fewer, once the maintainer overruled an attempt to close it by disposition rather than by actually fixing what was found — on top of the ≈3.1× the phase's own feature (`observability_reliability`) already cost against #7's first-pass approval.
