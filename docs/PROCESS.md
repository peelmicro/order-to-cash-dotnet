...
| `init.sh` | "Is the world sane?" — the session circuit breaker | Exit ≠ 0 ⇒ do not advance. Checks env, harness files, agent model declarations, backlog and SDD coherence, **plus two checks written here**: no superseded rule phrasing survives anywhere, and the session file names the active feature | Copied from #7; environment section rewritten for the .NET SDK, backlog validator kept as-is; two sections added in Phase 8 | Phase 2 | Phase 8 |
...
**Position: Phase 8 in progress — 17 of 43 features done.** The order aggregate exists, the in-process command bus exists, facts leave the write model the only way they are allowed to — written into an outbox row in the same transaction as the aggregate, published by a relay that stamps a row only after the broker acknowledges it — and, as of this feature, **the service runs**. An order can be placed over the message bus: a request arrives on the acceptance subject, reference data is resolved, stock is checked synchronously against a service that does not exist yet, a human-readable reference is allocated under a row lock, and the aggregate and its outbox row commit together.

The phase settled the trilogy's oldest open technical question, and settled it with a number. The previous assessment claims outbox rows with a database feature this engine does not have; the proposed equivalent was written into the design as a proposal, and the task list was written to **measure** whether it skipped contended rows or blocked on them rather than to assert the hoped-for answer. It skips — a second relay claimed a different row in 117 milliseconds while the first held its claim open indefinitely — and removing the one lock hint that does the work makes the same test block instead. The fallback the design had prepared was not needed. That question had been open since the first phase.

Three findings this phase are about the machinery rather than the product, and they escalate:

- The specification author found that the **containerised test databases had been created with a different isolation level from the deployed ones**, for six features. Harmless until a concurrency result depends on it, which is exactly what this feature was about to measure. Fixed before anything was measured.
- The implementer found that **its own first-draft tests did not exercise the production code for the side being measured** — the guard would have passed with the mechanism deleted. It said so and rewrote them. First time in this build an agent caught that class in its own work.
- The reviewer's forensics found a **zero-byte source file shadowing a real one**, traced by file permissions and timestamp to the coordinator's own verification tooling, which restored from a backup it had never taken when given a mis-typed path.

The last is the one to keep. **Nothing automated caught it.** Fourteen architecture rules, a full container-backed suite and the state-coherence script all passed with a phantom file in the tree. What caught it was somebody reading the working tree and asking why one file's mode looked wrong.

The acceptance feature that followed took **three review rounds — the first three-round feature of the build — and not one of the seven defects was a wrong line of code.** Every one was the same shape: correct behaviour that nothing would have noticed being removed. The second round is the sharp one, because it rejected the *first round's own fixes* for arriving unguarded — reverting the very symptom round one existed to eliminate left the whole suite green.

That is worth stating plainly against the benchmark, because it is not flattering in the obvious direction. **Two of the three blocking defects appear in the previous assessment's record for the same feature as advisories it disclosed and shipped.** The review here did not find harder things; it found the same things and refused to pass them. The extra round and a half is the price of that policy, and what it bought is seven armed guards where the previous assessment shipped two unarmed ones.
