...
| `feature_list.json` | "What is happening right now?" — the backlog state machine | 52 features, 8 `sdd: true`. Max one `in_progress`, enforced by `init.sh`. Only the reviewer sets `done` | #7's ids, names and phases **reset to `pending`**; one new feature (`cqrs_dispatcher`) | Phase 2 | every feature transition |
...
**Position: Phase 9 in progress — 22 of 52 features done, one feature left in the phase.** The second service exists. Stock can be reserved and released over RPC, the reservation lifecycle holds its invariant under concurrency, and Fulfillment publishes its own facts through its own outbox.

**The phase opened by closing the previous one's outstanding claim.** Four commands had sat parked in the Orders database since Phase 8 — six attempts each, "no responder" — because the design said a sweeper would resume them unattended once a service existed to answer, and nothing could yet demonstrate it. Starting the new service resumed all four with no operator action, confirmed by direct query of the running databases during review. A design claim that had to wait a phase for its evidence, and got it.

**The backlog keeps growing, and that is the process working rather than failing.** Four more entries were filed from this feature's declined advisories, all in the reviewer's own words. None is a defect today; each is a property asserted by a comment and held by no test, in code that four more services are about to copy. Filing them is cheaper than the alternative, which is discovering them once they have been copied four times.
...
| 9 | Fulfillment — stock reservations and DESADV creation | 🚧 the stock service is done: aggregate, reservation lifecycle, RPC responder, its own outbox — and the previous phase's parked saga commands resumed unattended the moment it started | ⬜ |
