...
| `feature_list.json` | "What is happening right now?" — the backlog state machine | 44 features, 8 `sdd: true`. Max one `in_progress`, enforced by `init.sh`. Only the reviewer sets `done` | #7's ids, names and phases **reset to `pending`**; one new feature (`cqrs_dispatcher`) | Phase 2 | every feature transition |
...
**Position: Phase 8 in progress — 18 of 44 features done.** The saga runs. An order placed over the message bus now advances across its whole lifecycle: a Kafka consumer turns each fact into one transactional unit, the owed command is enqueued in that same transaction and issued only after it commits, exhausted commands park durably and a sweeper retries them indefinitely, and both compensation paths work — a stock rejection cancels outright, a credit rejection releases the stock first and then cancels.

The backlog grew by one during the phase rather than shrinking: the review of the orchestrator found a real defect in **already-closed, already-committed work** — the order-number allocator's self-seeding check takes no lock, so two genuinely concurrent first-ever allocations can both lose the race. It is now a backlog item rather than a comment in a test, and its acceptance criteria require the test that currently proves the race **inverted rather than deleted**, because deleting it would remove the only evidence a fix worked.

**This feature was by a wide margin the most expensive in the build, and the reason is the finding.** Four implementation passes and four review rounds, against the previous assessment's single pass approved first time — roughly three times its total. Every defect the reviewer found across those four rounds was **in test code**. The orchestrator itself — the step table, the fourteen fact rows, the compensation mapping, the redelivery argument — was transcribed from the previous assessment and was never touched by a review finding. The single defect in production code was found by the implementer arming its own work.

That is the sharpest thing this repository has measured so far: **reuse moves the residual risk from "did we model it right" to "does our proof actually prove it" — and a wrong proof is green.** A specification tells you what to build and can be copied. It cannot tell you that the test you wrote to prove you built it is watching the wrong thing.
