...
| `feature_list.json` | "What is happening right now?" — the backlog state machine | 70 features, 8 `sdd: true`. **Twenty-eight** of them (ids 44-71) were filed by this run itself, all but one by its own reviews — a backlog that grows because the detector improves, not because the work does. Max one `in_progress`, enforced by `init.sh`. Only the reviewer sets `done` | #7's ids, names and phases **reset to `pending`**; one new feature (`cqrs_dispatcher`) | Phase 2 | every feature transition |
...
**Position: Phase 13 complete — 50 of 70 features done.** **All six services exist and run.** The Gateway serves REST per the copied contract, hand-rolled JWT, login rate limiting, MongoDB-only reads and a server-sent-event stream with heartbeat and replay — adding **no new package** to the dependency graph, where the predecessor needed three. The phase also closed six backlog entries, all of one class, and raised the trilogy's second shared-spec amendment.
