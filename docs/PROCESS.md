...
**The ruling paid for itself inside the same feature, and this is the part worth keeping.** Making the two excluded files identical *removed a property*: the per-service client identifier the message broker uses to attribute connections. That identifier is the very reason the previous assessment had excluded that file from its own consistency family. An empty value does not fail — the library substitutes a default silently — so all six services would have reported under one name, and the loss would have surfaced as confusing metrics rather than an error. It was caught, re-supplied as a required property, and armed by the compiler error a missing initialiser raises.

Had the recommendation been accepted, the property would have gone quietly. **Nobody writes a ledger row for a file they have excluded from the family** — the exclusion and the blind spot are the same act. This is the build's first observable save from the ported-idiom ledger, and it exists only because the gate refused a deferral.

**The honest caveat, which belongs in the final comparison:** you cannot observe a *prevented* defect. "It prevents" is an inference from one instance, not a measurement.
