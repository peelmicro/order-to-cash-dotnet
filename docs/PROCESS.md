...
| `CLAUDE.md` | "How do we do things here?" — binding conventions | Leader role, architecture non-negotiables, coding/testing conventions, commit discipline. **Amended at human gates as the build goes**, which is why nothing may quote the copy injected into its context | Copied from #7, **substantially adapted** — three rules translated, one added; amended repeatedly since. An amendment that *retires* an earlier phrasing also adds a line to `.superseded-rules` (4 lines there); one that only adds a convention retires nothing and adds none, so that file is a floor on the amendment count, not the count itself | Phase 2 | Phase 14 (checkpoint) |
| `feature_list.json` | "What is happening right now?" — the backlog state machine | 77 features, 8 `sdd: true`. **Thirty-five** of them (ids 44–78) were filed by this run itself — a backlog that grows because the detector improves, not because the work does. Max one `in_progress`, enforced by `init.sh`. Only the reviewer sets `done` | #7's ids, names and phases **reset to `pending`**; one new feature (`cqrs_dispatcher`) | Phase 2 | every feature transition |
| `init.sh` | "Is the world sane?" — the session circuit breaker | Exit ≠ 0 ⇒ do not advance. Checks env, harness files, agent model declarations, backlog and SDD coherence, **plus four checks written here**: no superseded rule phrasing survives anywhere, the session file names the active feature, a **backlog tripwire** that fails if a feature id disappears or a `done` reverts, and the **commit-message hook's presence**, reinstalled when missing because hooks are untracked | Copied from #7; environment section rewritten for the .NET SDK, backlog validator kept as-is; three sections added in Phase 8, a fourth in Phase 10, a fifth in Phase 13 — the shared-spec byte-parity check against #7, which nothing had guarded through two amendments | Phase 2 | Phase 10 |
...
| `specs/shared/` (7 files) | The system's definition, before the code | **Read-only.** A change is a spec amendment: explicit, human-gated, applied to every repository in the same session | **Copied verbatim from #7** — six of seven files byte-identical, `cmp`-proven | Phase 3 | Phase 13 (`SA-2`); `SA-3` approved and applied in Phase 14, commits **#8 `5ec5264` / #7 `5723874`** |
...
| `docs/PROCESS.md` | This document | Updated at the end of every phase — registry + status | Copied from #7, re-pointed; §11 reset | Phase 2 | Phase 25 |
| `README.md` | Honest front door at every commit | Grows incrementally each phase; never describes software that does not exist yet | Written here | Phase 1 | Phase 25 |
...
| `test.runsettings` | "Why does a full run die of file-watcher exhaustion?" — every real host a test builds opens a configuration file watcher, and a parallel suite runs out of the per-user inotify instances | Delivered to every test project by one `RunSettingsFilePath` line in `Directory.Build.props`; sets `DOTNET_hostBuilder__reloadConfigOnChange=false`. Guarded by a unit test that builds a real host and reads **the host's own configuration**, never the environment, armed by deleting either line and under the coverage run's own invocation | **Written here** — the predecessor's test processes build no .NET host, so it has no counterpart | Phase 14 | Phase 14 |
...
| `scripts/capture-demo.mjs` + `docs/screenshots/demo-compensation.gif` | The README's proof, not just its claim | Two Playwright browser contexts (one logs in off-camera, one records only the compensation demo), `ffmpeg` palette-based GIF encoding. Ran clean on its first attempt against a real stack | Ported from #7, adapted at three sites: #8's direct `accepted-order-link` navigation (no intermediate order-list page), #8's exact button copy, and #8's own `--env-file-if-exists` convention in place of #7's hand-rolled `.env` parser | Phase 24 | Phase 24 |
...
**Position: Phase 25 COMPLETE — the trilogy's #8 assessment is finished, 110 of 110 features done.** Counted: `python3 -c "import json;d=json.load(open('feature_list.json'));print(len(d['features']), sum(1 for f in d['features'] if f['status']=='done'))"` → `110 110`, on 2026-09-18. There is no phase 26; the next work on this repository, if any, is maintenance or a specific maintainer request, not a scheduled phase.

Id 38, the final checkpoint: `CHECKPOINTS.md`'s own first walk (an Opus reviewer, `progress/review_final_checkpoint.md`) correctly refused to close it — 7 of 40 boxes empty, verdict recorded plainly as "NOT YET SATISFIED." Every finding was worked in the same close-out, not smoothed over: two malformed/stale spots in `specs/shared/test-matrix.md` (a table-breaking unescaped `|`, a self-contradicting stale sentence); seven `done` features with no `progress/history.md` effort record, now all written; five of six rows in README's benchmark table citing the wrong phase's real figures, corrected against each phase's own primary total — the build's actual largest gap (`observability_reliability`, phase 14, ≈3.1×) had gone missing from the table entirely and is in now; only 1 of 4 n8n workflows ever actually fired against the real Gateway, the other three fired for real this close-out rather than deferred; a git-history spec-before-code ordering gap accepted with evidence, with one paragraph added to `docs/lessons.md` for #9. Six pre-existing pending backlog entries found at the start of the phase (52, 106, 107, 108, 109, plus two the id-52 audit itself surfaced, 110 and 111) were all disposed alongside the checkpoint walk — real fixes, leader-direct config edits, or accept-with-evidence dispositions, each recorded in `progress/history.md`.

**Not yet committed or pushed** — phases 24 and 25's combined work sits on disk awaiting the maintainer's "full wrap-up."

> The paragraph below this note describes phase 24's position and is kept as history.

**Position: Phase 24 COMPLETE, 1 of 1 entry closed, 102 of 108 features done.** Counted: `python3 -c "import json;d=json.load(open('feature_list.json'));print(len(d['features']), sum(1 for f in d['features'] if f['status']=='done'))"` → `108 102`, on 2026-09-18.

Id 37, documentation/demo/benchmark: mixed shape, mostly leader-direct docs work (README's Architecture, Trade-offs, Assumptions, Production-extensions, AI-process and Benchmark sections, the Spec-amendments back-port status), one LIGHT implementer session for the demo-GIF capture, no separate review. The demo GIF ran clean on its first attempt against a freshly-seeded real stack and was frame-verified, not just size-checked, to show the `.99` compensation saga end to end including the causal link on the cancellation entry. The benchmark section reads both repositories' `progress/history.md` in full rather than a sample, and states plainly what did not speed up: early phases were genuinely slower than #7, and the later speedup is attributed to reused review *findings*, not faster raw implementation. Two factual errors were caught and corrected before shipping by re-deriving the claim with a live command rather than trusting a first draft — a false feature-id citation, and a miscounted number of full-process reviews.

**Population check habit, applied again here.** Before writing the phase 25 brief, the leader re-ran the population count live rather than assuming "one entry left": 6 of 108 entries are still `pending`, not 1 — id 38 (phase 25 itself) plus five pre-existing backlog entries (ids 52, 106, 107, 108, 109) filed in earlier phases but never required for those phases to close. Phase 25's brief asks the next session to decide their disposition explicitly rather than let them go silently unaddressed past the trilogy close.

> The paragraph below this note describes phase 23's position and is kept as history.
...
- **Id 31, API tests:** full process, with one Sonnet implementer and one Opus review. It was approved on the first review round, with one Status-cell correction.
- **Ids 104 and 105:** light changes. Each had one implementer; the leader read the diff, ran the affected tests and re-armed the fix.

The phase took three implementer sessions and one review, and had no rejections.

> The paragraph below this note describes phase 16's position and is kept as history.
...
**Three remain open because they are the point of the exercise:** id 75 and id 79 (#7 does not conform to `SA-3` and `SA-4` respectively, and cross-repository conformance is the parity claim this trilogy exists to make) and id 86 (a delegation guard that passes while a service's health probes are unwired). The earlier checkpoint commit (`d8d71c7`) was taken mid-phase on the maintainer's word; this close is `b453931` plus the disposition commit that follows it. It carries **three entries approved at review** — observability and reliability (feature 27: retry then dead-letter, W3C trace propagation through NATS, Kafka and the write database, structured logs with the correlation id, five metrics, liveness and readiness on all six services, and client-idempotent order creation), the operator's cancellation note surviving the compensation branches (id 71), and a guard for a test-host file-watcher limit (id 77) — and **one entry committed as unreviewed work in progress** (id 62), labelled as such in the commit body. Suite at the checkpoint: **1833 tests, 0 failed**, counted from the last full quality run on that exact code.

**What phase 14 still owes, updated at this checkpoint.** The order stated at the previous checkpoint (id 62 → 73 → 76 → 72 → the 67–70 loop with 74 → 78) is **discharged as far as id 68**: ids 62, 73, 76, 72, 67, 68, 80 and 83 are closed. The 67–70 loop was **split into four slices** by the leader, because sizing it by reading rather than by impression showed five entries carrying roughly eight distinct defect classes, and the phase's own evidence was that one class per feature already cost two to five review rounds.

**What remains after the scope close: three entries, and they are all cross-repository or false-green work.** Id 75 — #7 renders the dispatch-*entry* instant into `x-first-failed-at` at three sites (`orders:136`, `projector:126`, `notifications:123`), which `SA-3`'s wording explicitly forbids; its orders spec pins the wrong meaning and its projector and notifications copies have no spec asserting the header at all. Id 79 — #7's code still releases the credit hold before the stock on a confirmed-order cancellation, after `SA-4` said otherwise in both repositories' specs. Id 86 — the composition-root delegation guard anchors a call *shape* rather than the one live host call, so a same-shaped decoy passes while Billing's health probes are unwired.

Id 87 was closed **without a review round**, at the gate, to conserve budget: the fix is armed both ways, its class was enumerated across every `CreateExecutionStrategy` and `UseSqlServer` site, and it was committed in `b453931` labelled as implemented-but-not-reviewed rather than presented as reviewed.

**`SA-3` is applied; nothing waits on it.** It gave `asyncapi.yaml` descriptions for the two dead-letter failure-time headers — `x-first-failed-at` (the instant the first processing attempt failed, never the instant processing began) and `x-failed-at` (the instant the final attempt failed) — at `asyncapi.yaml:2227-2229`. Commits: **#8 `5ec5264` / #7 `5723874`**. Id 75 was filed before that landed and still read as though the contract were silent; corrected at this checkpoint, it is now ordinary #7-alignment work needing no gate. **`SA-4`** followed in the same phase, making a cancellation release the contested stock reservation first: **#8 `1affd4a` / #7 `63f130e`**.

**No phase ratio is written at a checkpoint.** The per-feature figures are in `progress/history.md` and are quoted, not re-derived: feature 27 ran **≈24 h 10 min against the predecessor's ≈7 h 45 min (≈3.1×)** on a feature the predecessor approved first time, and its review/fix loop alone (≈9 h) cost more than the predecessor's whole feature. Ids 71 (≈4 h 08 min) and 77 have no counterpart in the predecessor at all.

**Phase 13, for the record: all six services exist and run.** The Gateway serves REST per the copied contract, hand-rolled JWT, login rate limiting, MongoDB-only reads and a server-sent-event stream with heartbeat and replay — adding **no new package** to the dependency graph, where the predecessor needed three.
...
**Phase 13 — the phase where the defect class was found five times and shipped zero times.** Five of its six rejections were one shape: **a guard that could not fail.** A route sweep that selected its candidates by the metadata under test. A ledger row naming a test that never called the code it described. A wire-key theory comparing a hand-typed list against the specification and never against the record. A config helper whose test supplied the very variable name whose *choice* was the thing at risk. A completeness claim written as prose where a search result was required — inside the loop whose whole subject was that class.

**Every one was found by a reviewer's mutation, not by anyone reading.** That is the number worth carrying: on this evidence, reading finds none of them. The reviews cost real time, and what they bought is that none of the five reached a commit.

**And the class kept teaching the same lesson from new angles.** A guard can be written correctly, assert the right field, and still be unable to fail if the *fixture* gives two fields values the assertion cannot distinguish — equality when the assertion compares equality, or one value being a **prefix** of another when it compares containment. A retry loop can pace itself on a timeout that never elapses, because the error it retries on is returned instantly. And *"no guard is needed here"* is itself a claim: one row concluded correctly that a property was framework-supplied, and its stated evidence could not fire — breaking the mechanism changed a 1 m 35.5 s run into a 1 m 35.4 s one.

**Phase 13 — a third mutation family, and the one that crosses service boundaries.** Deletion leaves behaviour missing. Corruption leaves data wrong. **Substitution leaves a working system pointed at the wrong target** — and it is the only one of the three whose green suite hides *correct behaviour aimed elsewhere*.

It was found because deletion and corruption both failed to apply: a configuration key is a source constant, so there is nothing to delete without a compile error and no value the test supplied to corrupt. Repointing one service's seed writer at another service's database left its suite **41/41 green** while it wrote fixtures into the wrong store.

The discriminator for when it applies is a property of the literal rather than of the code around it: **does it name a member of a set whose other members also exist in this repository?** A database name with three siblings is a genuine substitution; a connection-string flag with none degenerates into corruption. That makes the family mechanically enumerable — and, in a system of six services each owning a database, a topic set and a subject set, it is precisely the failure that crosses a boundary rather than staying inside one.

It has its own false negative, which any tool doing this must handle: if the substituted sibling is unset, the read falls back to a default and the test fails for the *default* reason rather than the *name* reason. **A swap that fails is not evidence until the failure message names what you intended to break.**

**Phase 13 — and the routing lesson, which cost two assessments to learn.** One acceptance criterion — an operator's cancellation note reaching the timeline — was disclosed honestly by the predecessor, disclosed honestly again here, and fixed by neither. Its review had written that *"the next feature that touches this payload must close it"*; the feature that touched it disclosed the gap a second time instead, correctly, because the shared specification is read-only without an amendment.

The evidence that turned this into a rule is that the stated reason for deferring **was not true when it was given**: the predecessor's own closing commit added 111 lines to that very file, so it was open, and the three lines that would have closed the gap were not among them — because that gap merely failed an acceptance criterion while the other work *blocked* the feature. The file was never touched again across 39 commits.

**Detection was never the failure. Routing was.** *"The next feature that touches X"* names nobody, so nobody is named. A disclosure whose root cause is the shared specification now leaves a numbered backlog entry or a named amendment proposal — never a sentence in a review — because a backlog entry is the only artefact in this harness that outlives the feature that found it.

> **Phase 14 entries below are recorded at checkpoints.** The phase is open. At the first checkpoint one entry concerned unreviewed work and said so; it has since been reviewed and approved, and is corrected in place below rather than left standing.

**Phase 14 — the defect entered at the brief, four times, and the briefs were the coordinator's.** Four acceptance criteria written into briefs in one feature each checked the wrong **unit**. A re-arm criterion keyed on the *group* of work that touched a file, when the claim was about the *arm* that touch made stale — 13 of 27 ledger rows had been touched after their arm, one of them hiding a real regression. A walk verified by *class* name, when a traceability row's unit is a *test case* — five row groups marked "exists" had no case. A walk scoped to a *reviewer's sample* of 19 groups, when the table had 41. And an arm accepted as "deterministic" without asking *which artefact* held the determinism.

The project already told agents to classify the unit a sweep's claim is about. All four failures were in criteria the **coordinator** wrote, which agents then applied faithfully — so the defect entered upstream of every check that could have caught it, and each check inherited it. The amendment adopted at the gate: **name the unit in every criterion a brief sets** (arm, case, row, site, copy, branch, ordering, invocation path); **a sample is evidence, never the population**, so give the command that counts the population; and **count the population yourself before dispatching a re-review**.

**Its sibling: a verification step a brief prescribes must be able to fail.** A brief told an implementer the build would catch broken cross-references in rewritten doc comments. Documentation generation is switched off in this repository, so that diagnostic is never emitted and the prescribed check could not fail. A doc-enabled build of one service found **12** unresolved references. It was filed as its own backlog entry rather than fixed at the two lines that happened to be noticed.

**Phase 14 — determinism belongs in the test, never in the mutation.** A concurrency guard was reported armed "deterministically". The delay that made the defect show had been inserted **into the mutation**, while both tests waited between their two calls and never overlapped at all — the plain defect passed 3/3. A mutation widened until the test fails proves that the assertion can read a defect the test itself never creates. The fix was a test-owned barrier that forces the overlap, and a standing instruction in the next brief: *if the named mutation does not fail, the test changes.*

**And its clock-shaped cousin: when a measurement moves from an injected clock to a stopwatch, its exact-value guard goes with it.** The predecessor asserted a processing latency of exactly `240` from a fake clock. The port measured with a stopwatch no test can drive and asserted `>= 0`, which recording ticks instead of milliseconds satisfied. Nothing about the behaviour was wrong on the tested path; the guard simply could not fail.

**Phase 14 — a background build is still a second build, and a pattern wait can wait on itself.** An arming rebuild racing a full quality run crashed a test host with a metadata error blaming a plain auto-property — incoherent line mapping, the signature of a half-written assembly rather than a defect. Two later occurrences involved **no second agent**: each time one agent started a long run in the background and kept working, partly because an earlier brief, written to stop agents stalling, said *keep going*. And one agent's wait, `until ! pgrep -f "quality.sh"`, never exited, because `pgrep -f` matches full command lines and the waiting shell's own line contains the pattern. Adopted: **while a build you started is alive, do only read-only work; wait on a process id, never on a pattern that can count the waiter.**

**Phase 14 — the environment changed under a running suite.** A full run lost 28 tests to *"Docker unavailable"*. The inner exception was a missing framework assembly: an **unattended package upgrade** had replaced the .NET host and runtime (10.0.11 → 10.0.12) and then the SDK (→ 10.0.112) seconds before the log stopped, and the Docker daemon had never restarted. The run was first attributed to Docker and corrected at review. A green re-run on identical code was accepted **only** because the red had an external cause with machine evidence — a package log and its timestamps — not because a later run was green. Before diagnosing a red full run, read the inner exception and the package log.

**Phase 14 — a harness setting is guarded by reading what the host receives.** On this machine the per-user inotify limit is 128 and desktop processes hold 113–115 instances at rest; one host-heavy test project peaked at 123 without the fix and 118 with it. The fix is one run-settings entry. Its guard builds a real host through the same composition root production uses and reads **the host's own configuration**, never the process environment — a test that read its own environment variable would prove only that the variable exists. It was armed on both lines that deliver it, and **under the coverage run's exact invocation**, because observing green on a path is not arming it.

**Phase 14 — the routing lesson, honoured the first time it applied.** Porting the retry dispatcher, the ledger's *"what did the predecessor rely on"* half was read out of its source line by line, and the line after the one being cited showed the predecessor's `x-first-failed-at` is the instant dispatch was **entered**, where this port records the instant of the first **failure**. The shared contract declared both headers with no description, so neither run violated it. Phase 13's rule applied: not a sentence in a review but a numbered backlog entry carrying a recommendation, which became **`SA-3`** at the human gate.

**Phase 14 (APPROVED at review round 2; this entry was written before the review and is corrected here rather than rewritten) — a defence that acts after acquisition.** Id 62 addresses an operator cancel racing the saga's own forward progress. Its order-row lock was proven in both orderings with no deadlock, and is kept. Its second defence ignored a genuine forward-progress fact whenever a compensation was pending — **enqueueing no compensation for the resource that fact reports acquired**, so a held credit or a completed despatch would be stranded in another service. The green tests could not see it: the stand-in responders answered success unconditionally, and no test observed a resource held anywhere else. The coordinator found it **reading the code rather than the record**, and sent it back before review. The predecessor's own named defence acts **before** acquisition — a transactional status re-check before dispatching — and the rework follows that shape: a dispatch gate, cancel planning that knows which forward commands are in flight, and stand-ins that record what they were asked to do. It was approved at review round 2, and the rework took the shape described: a dispatch gate, cancel planning that knows which forward commands are in flight, and stand-ins that record what they were asked to do.

**Phase 14 — a defect that had passed three consecutive runs was turned into a test that fails every time.** A SQL Server deadlock victim escaped `OutboxRelay.RunOnceAsync`; it failed two full-suite runs, then passed three, and never failed when its project ran alone. The temptation was to call it a flake. Instead the natural two-relay race was shown **structurally unable** to deadlock at the claim step — `WITH (UPDLOCK, READPAST, ROWLOCK)`, and both relays scanning `ORDER BY seq ASC` in the *same* direction, so no cycle can form; confirmed by forty iterations against `system_health`'s deadlock-graph ring buffer with zero hits. The real mechanism was the one statement carrying no lock hint, the stamp's `ExecuteUpdateAsync` — exactly where the original stack trace had pointed all along. A deterministic deadlock was then **constructed** against it, with `SET DEADLOCK_PRIORITY` making the outcome certain rather than probable. **"The flake stopped" is not evidence; a reproduction you built is.**

**Phase 14 — a guard written for another reason did the enumeration.** That same fix began as Orders-only. `OutboxRelayParityTests` (§8.3/§8.4) enforces that `OutboxRelay.cs` is identical across every service with a relational outbox, and it caught the incomplete fix and **named where the class actually ended** — Orders, Fulfillment and Billing. The enumeration rule this process keeps re-learning was, that once, enforced mechanically instead of remembered.

**Phase 14 — the phase had no stopping rule, and that is its defining finding.** It closed nine features and filed ten new entries; the backlog grew from 77 to 89 while the work itself went well. Every entry was real. The phase ended because the maintainer stopped it, and eleven entries were then dispositioned with documented evidence rather than worked. **"Is this defect real?" and "is this defect worth the budget?" are different questions, and only the first was being asked** — at 200k–800k tokens per closed entry, an open queue of fifteen is a multi-million-token commitment nobody ever decided to make, reached one reasonable filing at a time. The rule now lives in `CLAUDE.md`: an audit phase states its stopping rule before it starts, and every filed entry gets a *disposition* — fix, accept with evidence, or re-open only if X — because "pending" is a decision deferred, and deferred decisions accumulate silently since each one looks free.
