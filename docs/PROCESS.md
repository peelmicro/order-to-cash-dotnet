...
| `feature_list.json` | "What is happening right now?" — the backlog state machine | 50 features, 8 `sdd: true`. Max one `in_progress`, enforced by `init.sh`. Only the reviewer sets `done` | #7's ids, names and phases **reset to `pending`**; one new feature (`cqrs_dispatcher`) | Phase 2 | every feature transition |
...
**Position: Phase 9 in progress — 21 of 50 features done.** The second service exists. Stock can be reserved and released over RPC, the reservation lifecycle holds its invariant under concurrency, and Fulfillment publishes its own facts through its own outbox.
