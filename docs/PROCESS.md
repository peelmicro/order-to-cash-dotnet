...
| `feature_list.json` | "What is happening right now?" — the backlog state machine | 45 features, 8 `sdd: true`. Max one `in_progress`, enforced by `init.sh`. Only the reviewer sets `done` | #7's ids, names and phases **reset to `pending`**; one new feature (`cqrs_dispatcher`) | Phase 2 | every feature transition |
...
**Position: Phase 8 complete — 20 of 45 features done.** The Orders service is finished: an order is accepted over RPC, its facts leave through a transactional outbox, a saga advances it across its whole lifecycle with both compensation paths, commands that fail terminally are told apart from commands that merely failed, and the counter that names orders can no longer race itself.

**The phase closed larger than it opened.** Three of its features were not on the original list: a hand-rolled command bus, and two defects found in already-closed work — the money-column widths and the allocator's seed race. Two more were filed for later phases from the same reviews. A backlog that grows during a phase is not a planning failure here; it is what a review process producing findings looks like, and every one of the five is a defect that would otherwise have shipped.

**What the phase measured, and it is the most valuable output of the build so far.** Three defects were one class: a property #7's engine, language or library supplied for free, dropped in a rendering that read as faithful — key ordering, numeric width, atomicity. **All three satisfied their requirement text exactly**, so requirement-to-test traceability could not see them, and arming could not either, because the behaviour was present and correct on the path the test took. The detection record was **nought for three**. That produced the phase's closing amendment: every design that ports a mechanism now records what made it correct in the original and who supplies that property here, with a guard test required wherever the answer used to be "the engine did".

Alongside it, the saga feature produced the sharper statement of the same idea: **reuse does not reduce risk so much as relocate it** — from *did we model the domain right*, which a copied specification answers, to *does our proof actually prove it*, which nothing inherited can answer. A wrong model usually fails loudly. A wrong proof is green.
