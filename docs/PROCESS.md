...
**Position: Phase 8 in progress — 16 of 43 features done.** The order aggregate exists, the in-process command bus exists, and facts now leave the write model the only way they are allowed to: written into an outbox row in the same transaction as the aggregate, and published by a relay that stamps a row only after the broker acknowledges it.
