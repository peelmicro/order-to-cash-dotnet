...
The ten request/reply records are transcribed into `Infrastructure/Messaging/Rpc/StockRpcPayloads.cs` — Fulfillment's own copy, not a reference to Orders' `SagaCommandPayloads.cs`, per the established rule that *RPC payloads live in the service that speaks them*. `StockRpcPayloadTests` holds the transcription to the wire by hand-retyping each schema's key list and asserting it against the record's `JsonWire`-serialised property names; one further case, added by `billing_credit`'s closure of backlog id 51 (`BC23`), asserts that retyped list against the set parsed from `asyncapi.yaml` as text, so the two cannot drift together silently.
...
| L13 | **An un-hinted in-transaction re-read is still current, not stale** — the second face of L1, added by feature `fulfillment_despatch` (backlog id 54) at `src/Fulfillment/Application/DespatchCreationService.cs:67`'s un-hinted `FindByOrderReferenceAsync` | InnoDB `REPEATABLE READ` gives a **transaction-scoped** snapshot, so #7 needed the explicit lock on every re-read regardless of ordering | The read is issued only **after** `LockForOrderAsync` is granted, and `EfCoreUnitOfWork.cs:30` opens `IsolationLevel.ReadCommitted` with `READ_COMMITTED_SNAPSHOT` **ON**, which makes the snapshot **statement-scoped** rather than transaction-scoped — so a statement issued after the lock is held is guaranteed to see the racing writer's commit, with no hint required on this particular read. **Not portable**: under a transaction-scoped snapshot (PostgreSQL `REPEATABLE READ`, #9's likely default) the same code would return a stale read and turn a correct idempotent repeat into a permanent `UNAVAILABLE` | `FS19` (feature `fulfillment_stock`, inherited) exercises the blocking half; no #8 guard exists for the statement-scoped argument itself — recorded here as a claim to verify, not a tested property, per `progress/review_fulfillment_despatch.md` §5's row *"L1, second face"* |
