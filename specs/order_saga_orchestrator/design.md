...
3. `ChannelSagaCommandSignal` (singleton) writes the reference to a **bounded** `Channel<SagaCommandRef>` (capacity 1 024, `BoundedChannelFullMode.DropWrite`, `SingleReader = false` — see point 4) and returns immediately. `Signal` is synchronous and non-blocking; the consume loop returns at once (SO10).
4. `SagaCommandDispatchWorker : BackgroundService` drains that channel through `OrdersSagaDispatchOptions.DegreeOfParallelism` (default 8) **parallel** consumer loops, each opening one DI scope per item and calling `SagaCommandDispatcher.DispatchAsync(orderId, command, ct)` — where the NATS issue and the SO4 retries actually happen. **Backlog id 80, corrected below** — this was originally ONE sequential loop; the paragraph after "the rejected alternative" below records why that was wrong and what replaced it.
...
**Correction (backlog id 80) — the single worker moved the same defect one stage later, and it stood uncorrected until this fix.** The "why the naive .NET translation is wrong" paragraph's own argument — awaiting the dispatch inline on the consume loop would serialise every fact behind whichever command's dispatcher call is currently blocked — is an argument against **serial awaiting of `dispatcher.DispatchAsync` behind a single point of consumption**, not specifically against doing so on the Kafka consume loop. The composition this section originally adopted moved that single point of consumption from the consume loop to `SagaCommandDispatchWorker`'s one `await foreach`, and left it serial there: one slow or absent responder for ONE order's command still stalled every OTHER order's fast-path dispatch behind it, for the same ~16.5s worst case (§6.2), only one stage later than the rejected alternative describes. Found by the leader verifying backlog id 62's fix (`Confirmed_ReleaseWins`'s own test comment recorded the consequence directly: it depended on `SagaCommandSweeper`, not the fast path, to deliver `stock.release` while `despatch.create` was in flight). The fix: `SagaCommandDispatchWorker` now runs `OrdersSagaDispatchOptions.DegreeOfParallelism` (default 8) **parallel** consumer loops over the same channel (point 4 above), each with its own DI scope and its own call into `SagaCommandDispatcher`; `ChannelSagaCommandSignal`'s `SingleReader` flag changed from `true` to `false` to match (point 3 above). No per-order affinity or serialisation was added: `progress/impl_saga_command_fast_path_is_head_of_line_blocked.md` enumerates every invariant this saga has that could plausibly depend on per-order dispatch ORDER and finds none that do — the one contested-resource race SA-4 anticipates (`saga.md` §4.3, "the despatch already requested") is explicitly arbitrated by Fulfillment's own lock regardless of arrival order, not by Orders' own dispatch sequencing, and `credit.release` is never enqueued until the triggering fact (`stock.released.v1`) is itself processed, so the compensation ordering "stock.release before credit.release" is enforced by the fact-driven state machine, never by this worker's drain order. `SO11`'s claim (§6.3) already made concurrent claims of DIFFERENT rows safe — the atomic conditional `UPDATE` was never a single-worker-only mechanism, it was simply never exercised concurrently by this worker before now.
