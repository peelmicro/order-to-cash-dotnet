...
| A reply body that is an `RpcError` | `SagaCommandTransportError` | yes — **for now** |
| A well-formed typed reply, any `outcome` including `rejected` | returned normally | n/a (SO6) |

The `UNAVAILABLE`-vs-`TIMEOUT` split is the same distinction feature 15 paid a blocking review defect to keep (`progress/history.md`, feature 15, D1) and it is preserved here by using two distinct exception types rather than one. **Feature 42 owns the next refinement** — splitting the `RpcError`-body row on its `code` into a terminal-business set and a transient set, and adding a `rejected` end state to `saga_commands`. This feature must therefore: keep the two exception types distinct, keep the `RpcError` classification in **one** place with a comment naming feature 42, and leave `saga_commands.status` able to carry a fourth token (it is `varchar(10)`; `rejected` is 8 characters). It must **not** implement 42's classification.
