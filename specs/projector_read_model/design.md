...
**Backlog id 100** (`timeline_money_reads_as_minor_units`, found 2026-09-17: the maintainer saw "Credit hold of 9 245 EUR approved" on a €92.45 order's timeline) corrected `MoneyFormat.Of(long minorUnits, string currency)` from an UNSCALED integer render (`16130, "EUR"` → `"16 130 EUR"`, which a human misreads as a hundred times the real amount) to one scaled by the currency's own ISO 4217 minor-unit exponent (SA-5): `9245, "EUR"` → `"92.45 EUR"`; `5000, "JPY"` → `"5 000 JPY"` (exponent 0, no decimal point at all); `12345, "BHD"` → `"12.345 BHD"` (exponent 3). Grouping stays a single hard-coded ASCII space on the integer part. Integer arithmetic only throughout — string slicing on the digit string, never `/ 100`, never `decimal`, never `ToString("N0")` with a culture (which would give `16,130.00` under `en-US` and `16.130,00` under `de-DE`). `DomainDecimalTests` already bans `decimal` from this namespace and stays non-vacuous for it here.

The exponent itself has no BCL source keyed on a currency CODE: `RegionInfo`/`NumberFormatInfo.CurrencyDecimalDigits` are keyed by REGION, and even inverting through every installed culture returns `CurrencyDecimalDigits == 2` for a JPY- or BHD-matching region on this machine's ICU data — decisively the wrong answer, not merely "a candidate to check". So `SharedKernel.CurrencyExponent` is a hand-written table sourced from the ISO 4217 published exponent lists, defaulting to 2. `MoneyFormat.Of` delegates entirely to `SharedKernel.MoneyText.Format` — the ONE implementation this repository's timeline summaries, the Seed's timeline fixtures (`SagaFixtures.cs`) and Notifications' `NotificationFormat.FormatMoney` all share, so a projected timeline entry, a seeded timeline entry and a notification email read alike for the same amount by construction. The seed/projector voice difference `PR16`/`PR44` recorded (`"Credit hold of 16130 EUR approved"`, no grouping) is CLOSED by this — both now render `MoneyText.Format`'s output; the seed's hand-written `detail` subset (unrelated to money) remains an inherited #7 difference.
...
| **L11** | `createIndex` is idempotent when options match, and raises a conflict when they do not | the server | the same server; the projector **fails to start** with the offending index name and the `dropIndex` one-liner, rather than continuing. **Corrected in the `projector_read_model` fix round:** a real `mongo:8.3.8` raises code **86** (`IndexKeySpecsConflict`), not 85 (`IndexOptionsConflict`) as originally written here, for `PR22`'s own scenario (an existing plain unique index of the same name, versus this one's partial filter) — a `partialFilterExpression` difference is apparently classified as a key-spec difference, not an options difference. The catch clause covers both codes and the thrown message reports whichever the server actually raised, rather than a fixed number | `ReadModelIndexesTests › PR22_RefusesToStartAgainstANonPartialIndexOfTheSameName_NamingTheIndex` |
| **L12** | The partial filter expression's stored form | #7 wrote the JS literal `{ orderReference: { $type: 'string' } }`, and its seed was **edited** to match | #8's seed landed first, and writes it through `Builders<OrderTimelineDocument>.Filter.Type(d => d.OrderReference, BsonType.String)` — which renders `$type` with BSON's **numeric** code, not the string alias. **Corrected in the `projector_read_model` second fix round (review defect D4, on top of D1's correct operational answer): the mechanism is comparison, not storage.** `mongo:8.3.8` treats `{ $type: "string" }` and `{ $type: 2 }` as the **same** partial filter when it **compares** an existing index's specification against a requested one — so neither creation order raises `IndexOptionsConflict` — but it does **not** normalise the alias when **storing** the index: `getIndexes()` returns whichever rendering actually created it. Verified both ways, directly against the server with `mongosh` and through the real driver: numeric created first (the seed's rendering) then a hand-built string-alias filter against it is **accepted**, and the stored `$type` reads back as the **numeric** code `2`; the string alias created first then the seed's numeric rendering against it is likewise **accepted**, and the stored `$type` reads back as the **string** alias `"string"`. The projector keeps building the filter via the **identical** `Builders<…>.Filter.Type(field, BsonType.String)` call the seed uses — not because a conflict was observed in either order, but because it is the cheaper, self-documenting form, and because depending on comparison-time equivalence beyond what is tested here is exactly the kind of engine behaviour a future MongoDB version could change | `ReadModelIndexesTests › PR39_TheSeedsIndexThenTheProjectors_AndTheProjectorsThenTheSeeds_RaiseNoIndexOptionsConflict`, `› L12_TheServerComparesTheTypeAliasesAsEquivalentButStoresWhicheverRenderingCreatedTheIndex_InBothCreationOrders` |
...
Three rows were the ones flagged for a reviewer to attack first, because each was believed to be a property that is **present and correct on the path a deletion probe takes** and only wrong under a condition the happy path never creates: **L9** (`ReturnDocument` default — every signal carries the previous status, and the count of signals is still right), **L12** (`$type` rendering), and **L31** (`AutoOffsetReset` — identical behaviour on a fresh topic, permanently incomplete on a populated one). **L9 and L31 held exactly as predicted** when reviewed. **L12 did not — it was wrong twice, in two different ways, across two rounds. The first implementation record answered it backwards: it claimed two renderings of `$type` produce incompatible stored index specs, so "neither service can start after the other". That hazard does not exist on a real `mongo:8.3.8`, in either creation order — the first fix round corrected the operational answer, but explained it with a mechanism that is also false: it said the server *normalises the alias to the numeric code when storing the index*. It does not. The equivalence lives in the server's **comparison** of an existing index's specification against a requested one, not in what gets **stored**: creation order determines the stored rendering — `getIndexes()` returns the alias if the alias created it, and the numeric code if the numeric rendering created it — and the second fix round (review defect D4) corrected the row and made its guard self-checking in both directions. The row's own guard (`PR39`) was never vacuous — it fails on a genuinely different partial filter (e.g. `Filter.Exists` in place of `Filter.Type`) — but the first version of `L12`'s own guard only ever ran one creation order, so it could not see this divergence between the two rounds' claims.** This is itself an instance of the class the ledger exists to catch, one level up, twice over: a ledger row is a countable claim about a symmetric situation, and a probe run in one direction only answers half of it. None of the three original rows is visible to `R<n>` traceability, and none is visible to emission-deletion arming; L12 additionally shows that a ledger row is not self-checking until its guard runs every direction the claim depends on — it is visible only to whoever actually runs the arming the row prescribes, in both directions, and reads the result rather than the prior expectation.
