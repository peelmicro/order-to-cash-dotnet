...
/// <see cref="INatsClient"/> member <c>NatsRpcClient</c> calls — never an
/// extension method — so intercepting it here intercepts the real call
/// site. Every other member is a plain pass-through to
/// <paramref name="inner"/>: this class changes nothing about the two
/// calls' construction, dispatch or headers, only WHEN each call's own
/// <c>RequestAsync</c> invocation is allowed to reach the real connection.
///
/// Implements the same logic as the Orders.IntegrationTests copy of the
/// same name for its own project context — deliberately duplicated per
/// project, matching how <c>StandInResponder</c>/
/// <c>StandInFulfillmentStockCheckResponder</c> already live per-project
/// rather than in a shared test assembly. Not byte-identical.
/// </summary>
