...
/// <summary>
/// <c>CreditSubjects</c> is derived from the spec, never retyped — reads
/// <c>specs/shared/asyncapi.yaml</c> as TEXT (the discipline
/// <c>tests/Orders.UnitTests/RpcSubjectsTests.cs</c> already establishes)
/// and extracts each <c>billing.credit.*</c> channel's own <c>address:</c>
/// line.
/// </summary>
public sealed partial class CreditSubjectsTests
{
    [Theory]
    [InlineData("creditHold", CreditSubjects.CreditHold)]
    [InlineData("creditRelease", CreditSubjects.CreditRelease)]
    [InlineData("creditList", CreditSubjects.CreditList)]
    public void CreditSubjects_EqualTheAsyncApiChannelAddress(string channelKey, string expectedSubject)
    {
