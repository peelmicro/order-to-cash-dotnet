...
/// 67, phase 14) — <see cref="BillingDbContextFactory"/> is reached only by
/// <c>dotnet ef</c>'s reflection over <see
...
/// reads (<c>MSSQL_HOST</c>, <c>MSSQL_HOST_PORT</c>, <c>MSSQL_DB_BILLING</c>,
/// <c>MSSQL_APP_USER</c>, <c>MSSQL_APP_PASSWORD</c>) live and unguarded —
/// <c>dotnet ef migrations add</c>/<c>database update</c> is tooling this
/// team runs, so a wrong or missing read here points a migration at the
/// wrong database.
///
...
public sealed class BillingDbContextFactoryTests
{
    private static readonly string[] _envVars =
    [
        "MSSQL_HOST", "MSSQL_HOST_PORT", "MSSQL_DB_BILLING", "MSSQL_APP_USER", "MSSQL_APP_PASSWORD",
        "MSSQL_DB_ORDERS", "MSSQL_DB_FULFILLMENT", "MSSQL_DB_NOTIFICATIONS",
    ];

    private static IDesignTimeDbContextFactory<BillingDbContext> Factory => new BillingDbContextFactory();
    ...
    public void CreateDbContext_ReadsEveryVariable_WhenAllAreSetToNonDefaultValues()
    {
        ...
        try
        {
            ...
            Assert.Contains("Initial Catalog=custom_billing_db", connectionString, StringComparison.Ordinal);
    ...
    public void CreateDbContext_BuildsEveryDocumentedDefault_WhenOnlyTheRequiredPasswordIsSet()
    {
        ...
        try
        {
            ...
            Assert.Contains("Initial Catalog=otc_billing", connectionString, StringComparison.Ordinal);
    ...
    /// <c>MSSQL_DB_BILLING</c> has three siblings (<c>MSSQL_DB_ORDERS</c>,
    /// <c>MSSQL_DB_FULFILLMENT</c>, <c>MSSQL_DB_NOTIFICATIONS</c>) read by
    /// the three sibling <c>*DbContextFactory</c> classes. This test sets
    /// <c>MSSQL_DB_BILLING</c> AND every sibling to distinct, non-default
    ...
    /// </summary>
    [Fact]
    public void CreateDbContext_ReadsMsSqlDbBilling_FromItsOwnDistinctVariableName_NeverASiblingsKey()
    {
        ClearAll();
        Environment.SetEnvironmentVariable("MSSQL_APP_PASSWORD", "dev-password");
        Environment.SetEnvironmentVariable("MSSQL_DB_BILLING", "custom_billing_db");
