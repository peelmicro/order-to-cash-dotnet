...
/// <see cref="Measurements"/> while a concurrently running class drives the
/// real middleware on the SAME process-wide meter — e.g.
/// <c>RequestLatencyMiddlewareTests</c>' own two cases) is a genuinely
/// different thread from the one reading <see cref="Measurements"/>. A
/// plain <see cref="List{T}"/> is not safe under a concurrent writer, so
/// every write and every read takes <see cref="_gate"/>, and the reader
/// returns a SNAPSHOT (<c>ToArray()</c>) rather than the live list, so a
/// caller can never observe (or enumerate over) a collection still being
/// mutated.
/// </summary>
internal sealed class MetricCapture : IDisposable
{
    ...
    ///
    /// <para>
    /// The scope cannot be a tag: <c>otc_request_latency_ms</c> is tagged by
    /// <c>endpoint</c>, which a sibling class driving the same middleware for
    /// the same path would carry too. What every measurement DOES carry is the
    /// <see cref="ExecutionContext"/> of whatever code recorded it —
    ...
    /// before it acts flows into everything the test awaits and into nothing it
    /// does not. That is the scope: <see cref="OwnMeasurements"/> is the
