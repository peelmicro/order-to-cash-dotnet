...
/// <summary>
/// R60/OR6, design.md §8, tasks.md A4e — Notifications checks
/// <c>writeModel</c> (MS-SQL) and <c>factStream</c> (Kafka) only (design.md
/// §8.2 — Notifications owns no RPC client and no read model). This suite
/// pauses the REAL Kafka container and proves readiness reports
...
public sealed class HealthProbesTests(KafkaContainerFixture kafka, MsSqlContainerFixture mssql)
{
    ...
    /// for Notifications' 2 checks (<c>writeModel</c>, <c>factStream</c>)
    ...
    {
        var connectionString = await mssql.CreateFreshDatabaseAsync($"otc_notifications_health_{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            try
            {
                ...
                Assert.True(
                    ...
                    $"/health/ready took {readyInFlightElapsed.TotalMilliseconds:F0}ms while Kafka was paused, exceeding its {_pausedReadinessBound.TotalSeconds:F0}s bound (design.md §8.3: 2 checks x 2s + 2s margin) — the initial in-flight call.");
                ...
                while (DateTime.UtcNow < deadline)
                {
                    ...
                    Assert.True(
                        ...
                        $"/health/ready took {readyElapsed.TotalMilliseconds:F0}ms while Kafka was paused, exceeding its {_pausedReadinessBound.TotalSeconds:F0}s bound (design.md §8.3: 2 checks x 2s + 2s margin).");
                    ...
                    Assert.True(
                        ...
                        $"/health/live did not answer within {_pausedReadinessBound.TotalSeconds:F0}s while Kafka was paused (elapsed {liveElapsed.TotalMilliseconds:F0}ms).");
                ...
                Assert.Equal("down", downBody.Checks!["factStream"].Status);
                ...
                Assert.True(
                    ...
                    $"/health/live did not answer within {_pausedReadinessBound.TotalSeconds:F0}s while Kafka was paused (elapsed {livePausedElapsed.TotalMilliseconds:F0}ms).");
