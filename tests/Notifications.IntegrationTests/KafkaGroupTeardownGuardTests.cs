...
public sealed class KafkaGroupTeardownGuardTests
{
    ...
    /// compiler enforces at all 51 call sites.
    /// </summary>
    [Fact]
    public void TheHostHelper_DeclaresAGroupClearingWrapperAsItsReturnType_NotABareIHost()
    {
        var method = typeof(NotificationConsumptionTestSupport)
            .GetMethod(nameof(NotificationConsumptionTestSupport.StartHostAsync))!;

        // Task<(THost Host, string ConnectionString, FakeNotificationSender Sender)>
        var tuple = method.ReturnType.GetGenericArguments()[0];
        var hostType = tuple.GetGenericArguments()[0];

        Assert.True(
            hostType == typeof(KafkaGroupTestHost),
            $"NotificationConsumptionTestSupport.StartHostAsync declares its host as '{hostType}'. It must declare "
