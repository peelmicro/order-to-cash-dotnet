...
/// <summary>
/// R60/OR6, design.md §8, tasks.md A4e — Projector checks <c>factStream</c>
/// (Kafka), <c>rpcTransport</c> (NATS) and <c>readModel</c> (MongoDB)
/// (design.md §8.2 — Projector owns no write model). This suite pauses the
/// REAL MongoDB container and proves readiness reports <c>down</c> for
/// that check ONLY, liveness stays <c>200</c> throughout, and readiness
/// recovers on unpause.
/// </summary>
[Collection(ProjectorInfraCollection.Name)]
public sealed class HealthProbesTests(MongoContainerFixture mongo, KafkaContainerFixture kafka, NatsContainerFixture nats)
{
    ...
    /// for Projector's 3 checks (<c>factStream</c>, <c>rpcTransport</c>,
    /// <c>readModel</c>) is 3 x 2s = 6s. The further +2s margin is a FIXED
    /// per-request allowance, not scaled by check count — it pays for the
    /// HTTP round trip itself (Kestrel's own request dispatch, this
    /// suite's JSON body deserialization, loopback network/scheduling
    ...
    {
        var database = mongo.FreshDatabase($"health_{Guid.NewGuid():N}");
        ...
        var builder = ProjectorHost.CreateBuilder(
            ...
            configure: options =>
            {
                ...
                options.Mongo.Database = databaseName;
            },
        ...
        try
        {
            ...
            try
            {
                ...
                Assert.True(
                    ...
                    $"/health/ready took {readyInFlightElapsed.TotalMilliseconds:F0}ms while MongoDB was paused, exceeding its {_pausedReadinessBound.TotalSeconds:F0}s bound (design.md §8.3: 3 checks x 2s + 2s margin) — the initial in-flight call.");
                ...
                while (DateTime.UtcNow < deadline)
                {
                    ...
                    Assert.True(
                        ...
                        $"/health/ready took {readyElapsed.TotalMilliseconds:F0}ms while MongoDB was paused, exceeding its {_pausedReadinessBound.TotalSeconds:F0}s bound (design.md §8.3: 3 checks x 2s + 2s margin).");
                    ...
                    Assert.True(
                        ...
                        $"/health/live did not answer within {_pausedReadinessBound.TotalSeconds:F0}s while MongoDB was paused (elapsed {liveElapsed.TotalMilliseconds:F0}ms).");
                ...
                Assert.True(
                    ...
                    $"/health/live did not answer within {_pausedReadinessBound.TotalSeconds:F0}s while MongoDB was paused (elapsed {livePausedElapsed.TotalMilliseconds:F0}ms).");
        ...
        {
            await ProjectorTestHost.StopHostAndWaitForGroupToClearAsync(host, kafka);
        }
    }
}
