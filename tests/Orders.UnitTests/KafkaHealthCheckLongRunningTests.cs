...
public sealed class KafkaHealthCheckLongRunningTests
{
    ...
    /// directly against this exact address before writing the assertions
    /// below: the awaited <c>CheckAsync</c> call (this file's own first,
    /// throwaway run) took ~2003ms end to end via the health check's own
    /// 2s bound — confirming the address genuinely blocks rather than
    /// failing fast.
    /// </summary>
