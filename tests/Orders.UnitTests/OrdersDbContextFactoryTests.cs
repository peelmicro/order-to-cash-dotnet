...
/// 67, phase 14) — <see cref="OrdersDbContextFactory"/> is reached only by
...
/// reads (<c>MSSQL_HOST</c>, <c>MSSQL_HOST_PORT</c>, <c>MSSQL_DB_ORDERS</c>,
...
public sealed class OrdersDbContextFactoryTests
{
    ...
    public void CreateDbContext_ReadsEveryVariable_WhenAllAreSetToNonDefaultValues()
    {
        ...
        try
        {
            ...
            Assert.Contains("Initial Catalog=custom_orders_db", connectionString, StringComparison.Ordinal);
    ...
    public void CreateDbContext_BuildsEveryDocumentedDefault_WhenOnlyTheRequiredPasswordIsSet()
    {
        ...
        try
        {
            ...
            Assert.Contains("Initial Catalog=otc_orders", connectionString, StringComparison.Ordinal);
    ...
    /// <c>MSSQL_DB_ORDERS</c> has three siblings (<c>MSSQL_DB_FULFILLMENT</c>,
    /// <c>MSSQL_DB_BILLING</c>, <c>MSSQL_DB_NOTIFICATIONS</c>) read by the
    /// three sibling <c>*DbContextFactory</c> classes. This test sets
    /// <c>MSSQL_DB_ORDERS</c> AND every sibling to distinct, non-default
    ...
    /// </summary>
    [Fact]
    public void CreateDbContext_ReadsMsSqlDbOrders_FromItsOwnDistinctVariableName_NeverASiblingsKey()
    {
        ClearAll();
        Environment.SetEnvironmentVariable("MSSQL_APP_PASSWORD", "dev-password");
        Environment.SetEnvironmentVariable("MSSQL_DB_ORDERS", "custom_orders_db");
