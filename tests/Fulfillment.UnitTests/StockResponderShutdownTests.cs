...
/// directly via reflection — the only seam that reaches
/// <c>StopAsync</c>'s own logic without a real NATS connection.
/// </summary>
public sealed class StockResponderShutdownTests
{
    [Fact]
    public async Task BC22_CompletesShutdownWaitingForEveryInFlightRequest_WhenOneFaultsAndOneSucceeds()
    {
        ...
        var responder = new StockRpcResponder(
            ...
            Options.Create(new Fulfillment.Infrastructure.StockResponderOptions()),
        ...
        var inFlightField = typeof(StockRpcResponder).GetField("_inFlight", BindingFlags.NonPublic | BindingFlags.Instance)!;
        ...
        // must have observably run to completion before it returns —
        // both halves of BC22, because a "fix" that stopped waiting would
        // satisfy the fault-isolation half perfectly.
