...
/// #7's own SHARED copy across services. Drives the REAL
/// <see cref="KafkaDeadLetterPublisher"/> class through its
...
/// <c>NotificationDeadLetterTests</c> only asserted PRESENCE of
/// (<c>ContainsKey("traceparent")</c>); this class proves the header
/// carries the REAL active trace id, extractable back to it, and that no
/// header is fabricated when nothing is active.
/// </summary>
public sealed class KafkaDeadLetterPublisherTests
{
    ...
    /// publication's own known inputs. Armed by swapping <c>x-attempts</c>'s
    /// rendered value for a hard-coded wrong count — see the round-2 fix
    /// record.
    /// </summary>
    [Fact]
    public async Task PublishAsync_RendersEveryDeadLetterHeaderValue_NotJustWhetherItParses()
    {
        ...
        Assert.Equal(ConsumerNames.ToToken(ConsumerName.Notifications), headers["x-failed-consumer"]);
    ...
    private static DeadLetterPublication BuildPublication() =>
        new(
            ...
            FailedConsumer: ConsumerName.Notifications,
            Attempts: 3,
            Error: "boom",
