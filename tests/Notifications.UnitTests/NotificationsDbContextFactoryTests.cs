...
/// 67, phase 14) — <see cref="NotificationsDbContextFactory"/> is reached
/// only by <c>dotnet ef</c>'s reflection over <see
...
/// <c>MSSQL_DB_NOTIFICATIONS</c>, <c>MSSQL_APP_USER</c>,
...
public sealed class NotificationsDbContextFactoryTests
{
    ...
    [
        ...
        "MSSQL_DB_ORDERS", "MSSQL_DB_FULFILLMENT", "MSSQL_DB_BILLING",
    ];

    private static IDesignTimeDbContextFactory<NotificationsDbContext> Factory => new NotificationsDbContextFactory();
    ...
    public void CreateDbContext_ReadsEveryVariable_WhenAllAreSetToNonDefaultValues()
    {
        ...
        try
        {
            ...
            Assert.Contains("Initial Catalog=custom_notifications_db", connectionString, StringComparison.Ordinal);
    ...
    public void CreateDbContext_BuildsEveryDocumentedDefault_WhenOnlyTheRequiredPasswordIsSet()
    {
        ...
        try
        {
            ...
            Assert.Contains("Initial Catalog=otc_notifications", connectionString, StringComparison.Ordinal);
    ...
    /// <c>MSSQL_DB_NOTIFICATIONS</c> has three siblings (<c>MSSQL_DB_ORDERS</c>,
    /// <c>MSSQL_DB_FULFILLMENT</c>, <c>MSSQL_DB_BILLING</c>) read by the
    /// three sibling <c>*DbContextFactory</c> classes. This test sets
    /// <c>MSSQL_DB_NOTIFICATIONS</c> AND every sibling to distinct,
    /// non-default values before asserting, so repointing this factory's
    /// literal at a sibling's key fails on the WRONG DATABASE NAME this
    /// assertion names, never on a fallback-to-default reason (a swap that
    /// fails only because the sibling is unset would prove nothing —
    /// CLAUDE.md's named false-negative mode).
    /// </summary>
    [Fact]
    public void CreateDbContext_ReadsMsSqlDbNotifications_FromItsOwnDistinctVariableName_NeverASiblingsKey()
    {
        ClearAll();
        Environment.SetEnvironmentVariable("MSSQL_APP_PASSWORD", "dev-password");
        Environment.SetEnvironmentVariable("MSSQL_DB_NOTIFICATIONS", "custom_notifications_db");
        ...
        try
        {
            ...
            Assert.DoesNotContain("custom_fulfillment_db", connectionString, StringComparison.Ordinal);
