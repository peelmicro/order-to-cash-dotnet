...
/// <summary>
/// R60/OR6, design.md §8, tasks.md A4e — Billing checks <c>writeModel</c>
/// (MS-SQL) and <c>rpcTransport</c> (NATS) only (design.md §8.2 — no Kafka
/// check). This suite pauses the REAL NATS container and proves readiness
...
public sealed class HealthProbesTests(MsSqlContainerFixture mssql, NatsContainerFixture nats, KafkaContainerFixture kafka)
{
    ...
    /// for Billing's 2 checks (<c>writeModel</c>, <c>rpcTransport</c>) is
    /// 2 x 2s = 4s. The further +2s margin is a FIXED per-request
    ...
    {
        var connectionString = await mssql.CreateFreshDatabaseAsync($"otc_billing_health_{Guid.NewGuid():N}");
