...
/// `Fulfillment.IntegrationTests.MsSqlContainerFixture` (feature
/// db_fulfillment) exactly, retargeted at <see cref="BillingDbContext"/>.
/// <see cref="ReliabilityTableParityTests"/> also uses this same shared
/// container directly for the Orders/Fulfillment/Notifications side of the
/// comparison, so the whole cross-context check pays the container startup
/// cost only once.
/// </summary>
public sealed class MsSqlContainerFixture : IAsyncLifetime
{
    ...
    public BillingDbContext CreateDbContext(string connectionString)
    {
