...
/// cleanly" (the `otc_billing` half) — against a real MS-SQL container
/// (Testcontainers.MsSql), never a mock, and re-applies cleanly after being
/// dropped back to nothing, mirroring feature db_fulfillment's
/// `MigrationTests`.
/// </summary>
...
public sealed class MigrationTests(MsSqlContainerFixture fixture)
{
    ...
    {
        var connectionString = await fixture.CreateFreshDatabaseAsync($"otc_billing_migrate_{Guid.NewGuid():N}");
    ...
    {
        var databaseName = $"otc_billing_reapply_{Guid.NewGuid():N}";
