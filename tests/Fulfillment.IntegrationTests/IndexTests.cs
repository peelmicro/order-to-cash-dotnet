...
/// <summary>
/// Every index named in Databases doc §5 (`stock` unique
/// `(company_code, product_code)`; `reservations` `(order_reference,
/// status)`) plus the reliability indexes carried over from feature
/// db_orders (§4.3, byte-identical), read from <c>sys.indexes</c> /
/// <c>sys.index_columns</c> on the real database, not from EF's model.
/// Column order inside a composite index is asserted via
/// <c>SequenceEqual</c>.
/// </summary>
...
public sealed class IndexTests(MsSqlContainerFixture fixture)
{
    ...
    [
        new("stock", ["company_code", "product_code"], true),

        new("reservations", ["order_reference", "status"], false),

        new("despatches", ["despatch_reference"], true),
        new("despatches", ["order_reference"], true),
    ...
    {
        var connectionString = await fixture.CreateFreshDatabaseAsync($"otc_fulfillment_indexes_{Guid.NewGuid():N}");
