...
/// <summary>
/// R60/OR6, design.md §8, tasks.md A4e — Fulfillment checks <c>writeModel</c>
/// (MS-SQL) and <c>rpcTransport</c> (NATS) only (design.md §8.2 — no Kafka
/// check, Fulfillment publishes through its outbox but never consumes a
/// fact). This suite pauses the REAL MS-SQL container and proves readiness
/// reports <c>down</c> for that check ONLY, liveness stays <c>200</c>
/// throughout, and readiness recovers on unpause.
/// </summary>
[Collection(FulfillmentCollection.Name)]
public sealed class HealthProbesTests(MsSqlContainerFixture mssql, NatsContainerFixture nats, KafkaContainerFixture kafka)
{
    ...
    /// for Fulfillment's 2 checks (<c>writeModel</c>, <c>rpcTransport</c>)
    ...
    /// (observed live against THIS exact suite, `quality.sh` run,
    /// 2026-09-10: `TaskCanceledException` at the 100s mark). A request
    /// that does not complete in time is treated as "not observed this
    /// poll" HERE ONLY — a call made WHILE the dependency is paused below
    /// uses <see cref="TryGetTimedAsync"/> instead, and a timeout there is
    /// a FAILURE, never a skip, because both routes must answer within
    ...
    {
        var connectionString = await mssql.CreateFreshDatabaseAsync($"otc_fulfillment_health_{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            try
            {
                ...
                Assert.True(
                    ...
                    $"/health/ready took {readyInFlightElapsed.TotalMilliseconds:F0}ms while MS-SQL was paused, exceeding its {_pausedReadinessBound.TotalSeconds:F0}s bound (design.md §8.3: 2 checks x 2s + 2s margin) — the initial in-flight call.");
                ...
                while (DateTime.UtcNow < deadline)
                {
                    ...
                    Assert.True(
                        ...
                        $"/health/ready took {readyElapsed.TotalMilliseconds:F0}ms while MS-SQL was paused, exceeding its {_pausedReadinessBound.TotalSeconds:F0}s bound (design.md §8.3: 2 checks x 2s + 2s margin).");
                    ...
                    // catch), never skipped as "not observed".
                    ...
                    Assert.True(
                        ...
                        $"/health/live did not answer within {_pausedReadinessBound.TotalSeconds:F0}s while MS-SQL was paused (elapsed {liveElapsed.TotalMilliseconds:F0}ms).");
                ...
                Assert.Equal("down", downBody.Checks!["writeModel"].Status);
                ...
                Assert.True(
                    ...
                    $"/health/live did not answer within {_pausedReadinessBound.TotalSeconds:F0}s while MS-SQL was paused (elapsed {livePausedElapsed.TotalMilliseconds:F0}ms).");
