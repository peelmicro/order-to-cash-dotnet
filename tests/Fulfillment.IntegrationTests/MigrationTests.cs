...
/// <summary>
/// Feature db_fulfillment, acceptance 1: "migrations run from empty" —
/// against a real MS-SQL container (Testcontainers.MsSql), never a mock,
/// and re-applies cleanly after being dropped back to nothing, mirroring
/// feature db_orders's `MigrationTests`.
/// </summary>
...
public sealed class MigrationTests(MsSqlContainerFixture fixture)
{
    ...
    {
        var connectionString = await fixture.CreateFreshDatabaseAsync($"otc_fulfillment_migrate_{Guid.NewGuid():N}");
    ...
    {
        var databaseName = $"otc_fulfillment_reapply_{Guid.NewGuid():N}";
