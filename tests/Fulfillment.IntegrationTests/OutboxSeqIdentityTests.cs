...
/// feature db_orders's `OutboxSeqIdentityTests`.
/// </summary>
...
public sealed class OutboxSeqIdentityTests(MsSqlContainerFixture fixture)
{
    ...
    {
        var connectionString = await fixture.CreateFreshDatabaseAsync($"otc_fulfillment_seqid_{Guid.NewGuid():N}");
    ...
    {
        var connectionString = await fixture.CreateFreshDatabaseAsync($"otc_fulfillment_seqinc_{Guid.NewGuid():N}");
        await using var db = fixture.CreateDbContext(connectionString);
        await db.Database.MigrateAsync();
