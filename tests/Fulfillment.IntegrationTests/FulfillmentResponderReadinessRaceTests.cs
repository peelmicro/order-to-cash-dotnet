...
/// <summary>
/// Backlog id 63 — proves, deterministically, that <see cref="FulfillmentHostFixture"/>'s
...
public sealed class FulfillmentResponderReadinessRaceTests(NatsContainerFixture nats)
{
    [Fact]
    public async Task RequestSentBeforeAnythingSubscribes_ThrowsNoResponders_EveryTime()
    {
        ...
        var subject = $"stock.check.readiness-race-repro.{Guid.NewGuid():N}";
    ...
    public async Task WithTheWait_ADelayedSubscriberNoLongerLosesTheRace()
    {
        ...
        await FulfillmentHostFixture.WaitUntilReachableAsync(caller, subject, "probe"u8.ToArray(), CancellationToken.None);
