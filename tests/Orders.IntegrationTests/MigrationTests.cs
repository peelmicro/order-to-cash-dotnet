...
/// <summary>
/// Feature db_orders, acceptance 1: "migration applies against the composed
/// mssql container and re-applies cleanly from empty". Against a real
/// MS-SQL container (Testcontainers.MsSql), never a mock.
/// </summary>
...
public sealed class MigrationTests(MsSqlContainerFixture fixture)
{
    ...
    {
        var connectionString = await fixture.CreateFreshDatabaseAsync($"otc_orders_migrate_{Guid.NewGuid():N}");
    ...
    {
        var databaseName = $"otc_orders_reapply_{Guid.NewGuid():N}";
