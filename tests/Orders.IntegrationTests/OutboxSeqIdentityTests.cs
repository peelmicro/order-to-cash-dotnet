...
/// <summary>
/// Feature db_orders, acceptance 2 (the `outbox.seq` half): "`outbox.seq`
/// really is an IDENTITY and really increments" — both the column property
/// and the runtime behaviour, on the real database.
/// </summary>
...
public sealed class OutboxSeqIdentityTests(MsSqlContainerFixture fixture)
{
    ...
    {
        var connectionString = await fixture.CreateFreshDatabaseAsync($"otc_orders_seqid_{Guid.NewGuid():N}");
    ...
    {
        var connectionString = await fixture.CreateFreshDatabaseAsync($"otc_orders_seqinc_{Guid.NewGuid():N}");
        await using var db = fixture.CreateDbContext(connectionString);
        await db.Database.MigrateAsync();
