...
public sealed class KafkaGroupTeardownGuardTests
{
    ...
    {
        var method = typeof(SagaIntegrationTestSupport)
            .GetMethod(nameof(SagaIntegrationTestSupport.StartHostAsync))!;

        // Task<(THost Host, string ConnectionString)>
        ...
        Assert.True(
            ...
            $"SagaIntegrationTestSupport.StartHostAsync declares its host as '{hostType}'. It must declare "
