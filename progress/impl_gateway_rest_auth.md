...
| 6 | `PlaceOrderCommand.cs` | `issuedOrders.Record(reply.OrderId)` → `issuedOrders.Record(Guid.NewGuid())` | `PlaceOrderCommandHandlerTests.HandleAsync_RecordsTheReplysOrderId_InTheIssuedOrderWindow` | `Assert.True() Failure Expected: True Actual: False` | **[message updated by backlog id 82, re-run 2026-09-13]** the nameless message quoted immediately above is SUPERSEDED: the assertion was replaced by a self-describing one and this arm was re-run against the SAME mutation, on a forced `--no-incremental` rebuild. New verbatim failure: `the issued-order window does not hold the REPLY's own orderId c848a50c-513e-4fde-97e5-cc4dd551fffc. The handler recorded some other id, so GET /orders/c848a50c-513e-4fde-97e5-cc4dd551fffc would answer a false 404 right after placement.` | Yes — `cmp` identical, rebuilt, 3/3 green |
