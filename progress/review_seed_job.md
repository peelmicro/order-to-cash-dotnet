...
| C | `HeaderComplete = true` → `false` | `src/Seed/Infrastructure/Mongo/MongoSeedWriter.cs:133` | `Order_Timeline_Documents_Carry_Every_Field_With_The_Right_Types` **FAILED** at `SeedIntegrationTests.cs:179` (`Assert.True() Failure — Expected: True, Actual: False`) **[message updated by backlog id 82, re-run 2026-09-13]** the nameless message quoted immediately above is SUPERSEDED: the assertion was replaced by a self-describing one and this arm was re-run against the SAME mutation, on a forced `--no-incremental` rebuild. New verbatim failure: `the seeded timeline document for the completed order ORD-000001 has headerComplete=false. Every header field above was present, so the writer is not stamping the flag the read model uses to tell a complete document from a placeholder.` at `SeedIntegrationTests.cs:193`. NOTE: the line number in this row (`:179`) was stale — the killing assertion is `Assert.True(completed.HeaderComplete)`, not the `totalAmount` one three lines above it; re-located by re-running the arm rather than by reading the old number. | **CAUGHT** |
