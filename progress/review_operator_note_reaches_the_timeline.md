...
| D | optionality | `Summaries.cs` writes `detail["note"]` **unconditionally** (present-and-null instead of absent) | `SummariesTests.PR16_OrderCancelled` — `Assert.False() Failure` | **[message updated by backlog id 82, re-run 2026-09-13]** the nameless message quoted immediately above is SUPERSEDED: the assertion was replaced by a self-describing one and this arm was re-run against the SAME mutation, on a forced `--no-incremental` rebuild. New verbatim failure: `Summaries.OrderCancelled wrote a 'note' detail key (value: ) for a fact that carries no note. SA-2 requires the key to be absent, not present-and-null.` |
