...
| P10 | corruption, on the wire | same mutation applied to `R31`'s request instead | `tests/Fulfillment.IntegrationTests/StockCheckTests.cs:26` | `R31_AnswersPerLineWithoutMutatingAStockItemAndWithoutEmittingAFact` | **KILLED at line 30** — `Assert.True() Failure / Expected: True / Actual: False`, stack frame `StockCheckTests.cs:line 30` **[message updated by backlog id 82, re-run 2026-09-13]** the nameless message quoted immediately above is SUPERSEDED: the assertion was replaced by a self-describing one and this arm was re-run against the SAME mutation, on a forced `--no-incremental` rebuild. New verbatim failure: `the stock.check reply reports available=false for a request the fixture seeded with enough stock. Reply: {"Available":false,"Lines":null}.` at `StockCheckTests.cs:line 31`. This is the evidence for the `R31` judgement below |
