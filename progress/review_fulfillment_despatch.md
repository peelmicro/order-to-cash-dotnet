...
| **P4** | suppression | `DespatchCreationService.cs:70` — the in-flight-race branch replies `created: true` | `DespatchCreationServiceTests.F8_InFlightRace_AConcurrentCommitter…` | **FAILED** — `Assert.False() Failure` (1 failed / 102 passed) **[message updated by backlog id 82, re-run 2026-09-13]** the nameless message quoted immediately above is SUPERSEDED: the assertion was replaced by a self-describing one and this arm was re-run against the SAME mutation, on a forced `--no-incremental` rebuild. New verbatim failure: `the in-flight race branch replied created=true for despatch DES-000002, which a concurrent committer had ALREADY created. Only the winner may report created=true.` |
