...
> This feature shipped in Phase 8, before the ported-idiom ledger convention became binding at the Phase 8 gate. This row retroactively covers this feature's Kafka consumer envelope-decode boundary; nothing below changes shipped code. See `progress/impl_retroactive_ported_idiom_ledger_decode_boundaries.md` for the full audit.

| # | Boundary | #7 relied on | In #8 that property is supplied by |
|---|---|---|---|
| RL3 | Kafka consumer envelope decode — `SagaFactsConsumer.HandleMessageAsync` | `parseFactEnvelope` (`order-to-cash-nestjs/apps/orders/src/presentation/saga-facts.controller.ts:59-82`): JSON-parses a `Buffer`/`string` value, checks the result is an object, and checks all seven required envelope fields (`eventId`, `eventType`, `aggregateId`, `correlationId`, `causationId`, `occurredAt`, `payload`) are present — any failure throws `MalformedFactEnvelopeError`, caught by `route` (lines 148-162) and logged-and-acknowledged rather than redelivered, because a producer bug cannot be fixed by retrying | The same shape, hand-built: `JsonSerializer.Deserialize<Envelope<JsonElement>>` wrapped in `try`/`catch (Exception ex)` at `src/Orders/Presentation/SagaFactsConsumer.cs:76-95`, followed by `ValidateEnvelope` (lines 192 onward) checking the same required fields are non-empty/non-default, with the identical log-and-acknowledge (not redeliver) policy. No gap — the two field lists match one-for-one (`EventId`, `EventType`, `AggregateId`, `CorrelationId`, and further fields checked below line 209) |
| RL5 | NATS RPC client reply decode — `NatsSagaCommandsAdapter.SendAsync` (the saga's OWN outbound `stock.reserve`/`credit.hold`/`despatch.create`/`invoice.issue`/`stock.release`/`credit.release` calls — a sibling site to `NatsStockAvailabilityChecker`, RL1 above, built by this feature rather than `orders_acceptance`) | `nats-saga-commands.adapter.ts:170-178`'s `call<TRequest, TReply>`: `replyCodec.decode(reply.data)` wrapped in its own `try`/`catch`, rethrowing a malformed-JSON reply body as `SagaCommandTransportError` rather than letting a raw parse exception escape | **Confirmed gap — nothing.** `src/Orders/Infrastructure/Messaging/NatsSagaCommandsAdapter.cs:131-143` calls `RpcJson.IsErrorBody(reply.Data)` (which itself does `JsonDocument.Parse` — `src/Orders/Infrastructure/Messaging/Rpc/RpcJson.cs:28-35`) and `RpcJson.Deserialize<TReply>(reply.Data)`, neither inside a `try`/`catch (JsonException)`. A malformed reply body from a Fulfillment/Billing responder throws a bare, uncaught `JsonException` out of `SendAsync`, not the classified `SagaCommandTransportError` #7 guarantees. This is the identical shape feature 46 (`orders_stock_check_rpc_error_discriminator`) already fixed in the sibling `NatsStockAvailabilityChecker.CheckAsync` (RL1) — feature 46's own report explicitly scoped this sibling site OUT of its fix and named it for backlog id 52: `progress/impl_orders_stock_check_rpc_error_discriminator.md:102`, *"backlog id 52 exists to sweep the other, not-yet-touched pre-ledger boundaries ... and other services' RPC clients/responders"* |

**Gaps found:** RL3 is parity, confirmed on both halves. **RL5 is a real, confirmed #8-only gap** — see the backlog-entry candidate in `progress/impl_retroactive_ported_idiom_ledger_decode_boundaries.md`.
