...
| Q4 | **payload corruption on a different field** | `OrdersCreateResponder.cs:167` → `ProductPayload(…, p.Price.Currency, true)` — the `enabled` flag hard-coded on the wire | **KILLED.** `Assert.False() Failure — Expected: False, Actual: True`. **[message updated by backlog id 82, re-run 2026-09-13]** the nameless message quoted immediately above is SUPERSEDED: the assertion was replaced by a self-describing one and this arm was re-run against the SAME mutation, on a forced `--no-incremental` rebuild. New verbatim failure: `the catalog reply reports PROD-DISABLED as enabled=true. The responder is hard-coding the wire flag instead of carrying the row's own enabled column.` at `CatalogReferenceListAcceptanceTests.cs:220`. The new test guards the `enabled` payload field too, which nothing did before this round |
