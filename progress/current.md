...
**Feature:** none active — **id 75 `dead_letter_first_failed_at_semantics` closed `done` 2026-09-13**, both halves, no review round (human gate, to conserve budget): #7 now conforms to `SA-3` at all three dispatchers with guards written where two apps had none, and #8's own guard — which could not distinguish the entry instant from the first-failure instant — is armed. **Two entries remain open: id 79** (a real saga feature in #7, not alignment — four sites in `saga-steps.ts`, the compensation plan order, four specs asserting the old ordering, three armings) **and id 86**. Uncommitted: #7 has seven files at `63f130e`; #8 has the test and its record. Previously — **phase 14 closed to scope at the human gate, 2026-09-13.** The guard-hardening audit filed more entries than it closed, so the maintainer ruled that entries which do not change shipped behaviour are **dispositioned with documented evidence rather than worked**. Eleven were closed that way (ids 69, 70, 74, 78, 81, 82, 84, 85, 88, 89, 90 — each carrying an explicit ACCEPTED, NOT FIXED note and, where one exists, a re-open trigger). Id 87 was closed without a review round, also at the gate, to conserve budget. **Three entries remain open and are the real work:** id 75 and id 79 (#7 is non-conforming to SA-3 and SA-4 respectively — the parity claim is what the trilogy exists to make) and id 86 (a delegation guard that passes while a service’s health probes are unwired). Phase 14: 26 entries, 23 done, 3 pending. Backlog: 89 entries, 73 done.
