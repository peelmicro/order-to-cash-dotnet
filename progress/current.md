...
**Feature:** none in progress. **Phase 22 COMPLETE (2026-09-18)**: id 35 `observability_dashboards` (LIGHT, one implementer, closed by the leader with no separate review) is done. All five dashboard panels and a full 42-span, 6-service, depth-26 saga trace verified against real, freshly placed orders, with trace continuity confirmed at the database level (`trace_parent` on real outbox rows), not merely trusted from Jaeger's UI. One real bug found and fixed within scope: 4 of 5 panels queried Prometheus metric names with a `_milliseconds`/`_ratio` suffix .NET's OTel SDK never produces (unlike #7's JS SDK) — the dashboard's queries were corrected; `infra/grafana/dashboards/order-to-cash-overview.json` is the only file this feature touched. The underlying `src/` question (should the instruments instead gain a `unit:` to match #7's naming) is filed as id 109, not blocking. 100 of 108 backlog features are done (counted: `python3 -c "import json;d=json.load(open('feature_list.json'));print(len(d['features']), sum(1 for f in d['features'] if f['status']=='done'))"` → 108 100). Nothing from phase 22 is committed or pushed yet.

## Next phase — Phase 23: full Docker Compose (id 36)

**Brief for the next session.** Start it in a FRESH session, and apply CLAUDE.md's "Cost discipline". **Population check done**: `feature_list.json` has exactly one entry with `"phase": 23` — id 36.
- **Id 36 `full_docker_compose`** (sdd: false). Its acceptance bullets:
  - cold start reaches a demoable state within ~2 minutes;
  - infra-only mode still works with apps run from the CLI.
- **Classification: LIGHT-but-large** (infra/config, no saga/money/security touch — matches id 34's shape, not id 47's), but this is genuine, substantial CONSTRUCTION, not verification like phases 20/22 turned out to be. Budget accordingly; do not assume this is a quick phase. Re-classify to full only if evidence says otherwise.
- **Verified live by the leader — the honest current state:**
  - **Nothing exists yet.** `docker-compose.apps.yml` does not exist in #8. No service or web Dockerfile exists either (`find . -iname "Dockerfile*"` finds only `infra/otel-collector/Dockerfile` and `infra/kafka/Dockerfile`, both already used by `docker-compose.infra.yml`, unrelated to this feature).
  - #7's equivalent exists and is the porting reference: `../order-to-cash-nestjs/docker-compose.apps.yml` (399 lines, 13 service entries: a migrate-then-run container PAIR per service — `orders-migrate`/`orders`, `fulfillment-migrate`/`fulfillment`, etc. — plus `projector`, `gateway`, `web`, `seed`, and the network) and `../order-to-cash-nestjs/infra/docker/{service,web,seed}/Dockerfile`.
  - **#8's migration shape genuinely differs from #7's, and it is incomplete today — a real gap for this phase to close, not just port around.** #7 runs a separate migration container per service before its app container starts. #8's `Seed` project (`src/Seed/Presentation/SeedRunner.cs:29`) applies migrations for only THREE of the four SQL-backed services — its own log line says so verbatim: `"[seed] applying migrations (orders, fulfillment, billing)..."`. **Notifications has its own EF Core migrations** (`src/Notifications/Infrastructure/Persistence/Migrations/`) that nothing applies automatically today — the only existing path is the manual `pnpm db:migrate:notifications` (`scripts/dev-stack.sh env dotnet ef database update --project src/Notifications`) a developer has to remember to run by hand. Decide deliberately how the compose file handles this: extend Seed to migrate Notifications too (a `src/Seed` code change, technically outside this feature's normal "compose config only" scope — if you do it, say so explicitly and keep it minimal), or give Notifications its own migrate-then-run container pair like #7's pattern, or something else — but do not silently copy #7's four-service migration shape without accounting for the fact that #8's Seed currently only covers three.
  - `global.json` pins the .NET SDK to `10.0.111`, `rollForward: latestPatch` — the build stage of every service Dockerfile needs to match.
  - `docker-compose.infra.yml` already exists and works (`otcnet-*` container names, `otcnet-net` network) — `docker-compose.apps.yml` needs to join that same network and compose file set, not duplicate it (`docker compose -f docker-compose.infra.yml -f docker-compose.apps.yml up`, matching the pattern the root `package.json`'s `dc:up:sonar` etc. already use for multi-file compose).
- **Do, with evidence:**
  1. Read `../order-to-cash-nestjs/docker-compose.apps.yml` and its Dockerfiles in full before starting. Enumerate #7's services, healthchecks and `depends_on` chains as a search result (one line per service), not from memory.
  2. Write one Dockerfile per #8 service (Orders, Fulfillment, Billing, Notifications, Projector, Gateway) — multi-stage (SDK build, ASP.NET Core/runtime-only final stage), matching `global.json`'s pinned SDK version. Decide whether one shared Dockerfile with a build-arg for the project name works (cheaper to maintain, matches "the same shape" principle) or six near-identical files are clearer — record the choice.
  3. Write the web app's Dockerfile (Next.js production build + `next start`) — #7's own `infra/docker/web/Dockerfile` (already fixed this session's earlier phases to build `@otc/shared-kernel` first) is a useful shape reference even though the build tooling differs (pnpm/Next vs whatever #8's `apps/web` uses — confirm `apps/web`'s own build convention first, it's already established from phase 16).
  4. Write `docker-compose.apps.yml`: one service entry per component, healthcheck-gated `depends_on` so the six .NET services start only once their real dependencies (MS-SQL, Kafka, NATS, Mongo) are healthy, and the migration/seed step (however you decided to shape it in step 2 above) gates the six services' own startup.
  5. **Arm the acceptance bullets as countable claims, per `CLAUDE.md`'s arming protocol:**
     - cold start to demoable state in ~2 minutes — TIME it for real, from `docker compose ... up` with no image cache reuse claimed (state whether you measured with or without a warm image cache, and if warm, also give a cold-image number once);
     - "infra-only mode still works with apps run from the CLI" — bring up ONLY `docker-compose.infra.yml` (no apps compose file), then run at least one service via `scripts/dev-stack.sh` or a bare `dotnet run`, and confirm it works exactly as today's `scripts/dev-stack.sh start` already does. This must not regress; `scripts/dev-stack.sh` is not being replaced by this feature.
  6. Confirm the demoable state genuinely means something concrete: an order placed through the composed web app / Gateway actually reaches `completed`.
- **Environment:** for any image build, use a scratch `DOCKER_CONFIG`, never edit the maintainer's `~/.docker/config.json` — this phase builds MANY images, so this matters more than in prior phases. Tear everything down cleanly when done; confirm with `docker ps`/`pgrep`.
- **Stopping rule:** the phase closes id 36. Any new finding is filed with a disposition. If the maintainer asks for a full wrap-up, that comes first.
