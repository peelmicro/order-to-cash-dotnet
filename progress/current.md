...
- `## How this is being built — the AI process` (renamed from `## How this is being built`), with a `### The cost-discipline correction, mid-project` subsection; and `## Demo` (new, near the top, matching #7's placement) with the real captured GIF.
- `## Benchmark: #8 against #7, and what did not speed up`, built from both repositories' `progress/history.md` (read in full, not sampled), with an explicit "what was NOT faster" section.
- `## Spec amendments`'s back-port-status paragraph, verified live (`git log` in #7's checkout, `cmp` on `openapi.yaml`) — all five amendments confirmed landed in #7, by commit.
- The demo GIF (`docs/screenshots/demo-compensation.gif`) was captured live by a scoped LIGHT implementer session (brief: `scratchpad/brief_demo_gif.md`, report: `progress/impl_demo_gif_capture.md`), ran clean on the first attempt, and was frame-verified by the implementer (not just size-checked) to show the real `.99` compensation saga end to end.

Two factual errors caught and corrected by the leader before shipping, both by re-deriving the fact with a live command rather than trusting a first draft: a false "id 55" citation in the Trade-offs table (the id actually names an unrelated feature; reworded to cite only the genuinely-present `R55` requirement), and a miscount of full-process reviews in phases 18–23 (first draft said 1, `grep`-derived correction is 3 — ids 31, 32, 47).
...
- **Decide explicitly whether phase 25's "final checkpoint" scope includes disposing of the five leftover backlog entries (ids 52, 106, 107, 108, 109), or whether they are deliberately left open past the trilogy close with a recorded reason.** Read each one's own filed note first (id 108's is in `progress/history.md`'s "Id 108" entry above; the others need a fresh `feature_list.json` read) — do not assume they are all the same shape or all safe to defer. This is exactly the kind of population the project's own "audit-style work needs a stopping rule written before it starts" convention applies to.
- **This is the trilogy's #8 closeout.** Likely leader-direct, docs-and-checkpoint-shaped like phase 24, not a code feature — but confirm against the actual acceptance bullets rather than assuming.
- **A "full wrap-up" has not happened since phase 23** (`276344f`) — phase 24's work (README rewrite, `progress/history.md`, `feature_list.json`, the demo GIF, `scripts/capture-demo.mjs`, `package.json`) is all still uncommitted on disk. When the maintainer says the words, commit phase 24 and phase 25 together (or separately, per the maintainer's own commit-discipline rule of one commit per feature) — never assume which without asking, and never ask whether to commit.
- **Stopping rule:** the phase closes the backlog. Any new finding is filed with a disposition, not silently fixed. If the maintainer asks for a full wrap-up, that comes first, and per CLAUDE.md's "wrap-up request comes first" rule, new findings surfaced during that wrap-up are reported with a recommendation and a question — never silently turned into fix rounds.

## OLD — Next phase — Phase 24: documentation, demo, benchmark (id 37) [superseded, closed above]

**Brief for the next session.** Start it in a FRESH session, and apply CLAUDE.md's "Cost discipline". **Population check done**: `feature_list.json` has exactly one entry with `"phase": 24` — id 37.
- **Id 37 `documentation_demo`** (sdd: false). Its acceptance bullets:
...
- **This phase's SHAPE differs from every other phase this session — say so plainly, do not force it into the code-feature pattern.** It touches no `src/`, `tests/` or (mostly) `apps/web/` — it is writing and one real capture run. Much of it (README sections, the benchmark table, the spec-amendment list) is squarely within the LEADER's own direct-edit scope per `CLAUDE.md` ("you may edit docs, compose, `infra/`, `progress/`, `n8n/`, `scripts/` and root config yourself") — the next session's leader may write large parts of this itself rather than dispatching an implementer for everything. The demo-capture step needs a real running stack and browser automation, which is implementer-shaped (or the leader can run it directly via Bash, since it is a script invocation, not application code). Decide the split deliberately and record why, rather than reflexively dispatching one implementer for the whole phase.
- **Verified live by the leader — the honest current state:**
  - `README.md` today has NO architecture diagram, NO saga diagram, NO Kafka-vs-NATS matrix section, NO "what it looks like"/trade-offs/AI-process section, and no demo GIF exists anywhere in the repo (`find . -iname "*.gif"` — empty).
  - **#7's README is the structural reference** — read it in full first, section by section, before starting: `Demo`, `Quick Start`, `Why this project exists`, `What it looks like`, `Tech stack`, `Architecture`, `Prerequisites`, `Working with the monorepo`, `Running the infrastructure`, `Running the full stack in Docker`, `n8n demo workflows`, `End-to-end tests`, `Trade-offs`, `Assumptions, and what I would do differently`, `Scaling and production extensions`, `How this is being built`, `The specification`, `Build progress`, `Licence`. #8's README is currently much thinner (`The trilogy...`, `Spec amendments`, `Tech stack`, `Prerequisites`, `Repository layout`, `How this is being built`, `Build progress`, `Running it`, `Licence`) — decide which of #7's extra sections genuinely apply to #8 (architecture and saga are shared/stack-agnostic; some trade-offs are .NET-specific and need rewriting, not copying).
  - **The demo GIF is a real, working, portable script in #7**: `scripts/capture-demo.mjs` (`pnpm media:demo` in #7) uses Playwright (two browser contexts — one to log in without recording, one to record only the compensation demo) plus `ffmpeg` on `PATH`, and outputs `docs/screenshots/demo-compensation.gif`. #8 already has Playwright wired up since phase 19 (`apps/web/e2e/`, `@playwright/test` 1.62.1) — port this script, adapting only what genuinely differs (selectors, if #8's UI structure differs the way phase 19's port found — check `apps/web/e2e/compensation.spec.ts` for the current selectors before assuming #7's script's selectors carry over). There is also `scripts/capture-media.mjs` (`pnpm media:capture` in #7) — read what it does before deciding whether #8 needs it too.
  - **The benchmark table's source data already exists and is real**: every phase this session (and every prior phase) has an effort record in `progress/history.md`, in both repos, with session counts and wall-clock estimates, several with an explicit "against #7's baseline" comparison already written (ids 32, 33, 35, 36 all have one). Read every entry in BOTH repos' `progress/history.md`, not a sample — this is a completeness claim, and a benchmark table built from a sample would be exactly the "sample standing in for the population" mistake `CLAUDE.md` warns against.
  - **The "spec amendments listed with their back-port status" bullet already has its source data too**: `README.md`'s existing `## Spec amendments` section lists SA-1 through SA-5 (confirm the current count live, do not assume it is still 5) with a registry row each — read that section, and confirm each amendment's back-port to #7 actually landed (grep #7's own commit log / `progress/history.md` for each `SA-n`), rather than assuming the registry row's claim is still accurate.
- **Environment:** for the demo capture, `ffmpeg` needs to be on `PATH` — check first and record whether it needed installing. Bring the stack up (`pnpm stack:start` or `pnpm dc:up:apps`, your choice, record which) for the capture, tear it down cleanly afterward.
- **Stopping rule:** the phase closes id 37. Any new finding is filed with a disposition. If the maintainer asks for a full wrap-up, that comes first.
...
**Roslyn ends the class rather than the instance:** a `#if false` region becomes **disabled trivia** and never reaches the live syntax nodes — whitespace after `#` is the lexer's concern, so D12 cannot exist; raw strings, verbatim strings and interpolation holes are **literal tokens**, never arguments, so D13 and round 2's defeats cannot exist **[RETRACTED 2026-09-14 by backlog id 86 — this absolute is FALSE and was measured false. An interpolation-hole decoy and a positional-argument decoy are each 2/2 GREEN on the old guard and 2/2 RED on the corrected one, so those defeats demonstrably can and do exist. Id 86 bullet 3 required deleting this claim rather than weakening it, and its enumeration found THREE textually identical copies, not the two the entry named. Retained here struck through rather than deleted, because a sweep for the RETIRED wording is how the third copy was found — see `progress/impl_batch_d6_delegation_guard_anchors_the_live_call.md`]**; a named argument is an `ArgumentSyntax` with `NameColon.Name` read exactly, which also retires D3's fully-qualified-name false red; and a **dropped optional argument is an absent node**, making shape 7 structural instead of textual — the shape that beat the guard twice and covers **11 of the 19 rows** (`configureTelemetry` ×6, `configureHealth` ×5).
