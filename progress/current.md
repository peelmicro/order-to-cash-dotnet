...
**Feature:** none in progress. **Phase 23 COMPLETE (2026-09-18)**: id 36 `full_docker_compose` (LIGHT-but-large, one implementer, closed by the leader with no separate review — the leader independently re-verified the compose config, read both Dockerfiles in full, and confirmed the "apps file alone fails loudly" claim) is done. `docker-compose.apps.yml` + one shared per-service Dockerfile (parameterised by a `SERVICE` build arg) + a web Dockerfile, layered on `docker-compose.infra.yml`, not duplicating it. Also closed within scope: the Notifications-migration gap flagged in this phase's own brief (all four MS-SQL-backed services now get a migrate-then-run container pair — zero `src/` changes). Real evidence: cold start to all-18-healthy in 91.8s warm-image-cache (a true from-scratch build+start, ~167s, is disclosed separately as exceeding the ~2-minute figure, not hidden); infra-only mode re-confirmed unregressed via the real, unmodified `scripts/dev-stack.sh`; a real order (`ORD-000007`) placed through the composed Gateway reached `completed` and survived a full container restart. A note for a future session, not acted on: the `external: true` compose-network footgun this feature found and removed may also affect #7's own `docker-compose.apps.yml` — not investigated there, #7 stays untouched. 101 of 108 backlog features are done (counted: `python3 -c "import json;d=json.load(open('feature_list.json'));print(len(d['features']), sum(1 for f in d['features'] if f['status']=='done'))"` → 108 101). Nothing from phase 23 is committed or pushed yet.

## Next phase — Phase 24: documentation, demo, benchmark (id 37)
