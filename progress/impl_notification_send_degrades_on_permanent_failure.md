...
| 3 | A1 | `ActivityTrackingOptions.TraceId \| SpanId` → `ActivityTrackingOptions.None` (CLAUDE.md's own documented probe for this exact property) | `NotificationsHost.cs` | same integration test as above | `Assert.False() Failure / Expected: False / Actual: True` (`TraceId` vanished — `string.IsNullOrEmpty` became true) | **[message updated by backlog id 82, re-run 2026-09-13]** the nameless message quoted immediately above is SUPERSEDED: the assertion was replaced by a self-describing one and this arm was re-run against the SAME mutation, on a forced `--no-incremental` rebuild. New verbatim failure: `Assert.Matches() Failure: Pattern not found in value / Regex: "^[0-9a-f]{32}$" / Value: null` — the presence-only `Assert.False(string.IsNullOrEmpty(...))` pair was DELETED (the `Assert.Matches` a line later already subsumes it for both null and empty, and prints the offending value). | `cmp` identical; rebuilt; same test → `Passed! - 1, Total: 1` |
