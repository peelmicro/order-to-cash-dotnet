...
**Pure-logic guard (`DeadlockRetryExecutionStrategyTests`)**: mutated `ShouldRetryOn` to `=> false`, rebuilt, ran the five tests — the two "should retry" tests failed (`Assert.True() Failure — Actual: False` **[message updated by backlog id 82, re-run 2026-09-13]** the nameless message quoted above is SUPERSEDED; same mutation, forced `--no-incremental` rebuild. New verbatim failure: `ShouldRetryOn refused a RAW SqlException with error number 1205 (the deadlock victim). The outbox relay would surface it instead of retrying.` and `ShouldRetryOn refused a deadlock-victim SqlException (error 1205) WRAPPED in InvalidOperationException — the exact shape ExecuteUpdateAsync's own internal strategy produces, one level deeper than CallOnWrappedException unwraps.`), the three "should not retry" tests stayed green (expected: a false negative would not manifest through them). Mutated to `=> true`, rebuilt, reran — the three "should not retry" tests failed (`Assert.False() Failure — Actual: True` **[message updated by backlog id 82, re-run 2026-09-13]** the nameless message quoted above is SUPERSEDED; same mutation, forced `--no-incremental` rebuild. New verbatim failure: `ShouldRetryOn retried a RAW SqlException with error number 1222 (lock request timeout), which is NOT a deadlock. Retrying it would hide a genuine failure.`, `ShouldRetryOn retried a WRAPPED SqlException with error number 1222 (lock request timeout), which is NOT a deadlock.` and `ShouldRetryOn retried a plain TimeoutException carrying no SqlException at all — the predicate is not inspecting the error number.`), the two "should retry" tests stayed green. Restored from backup, `cmp` byte-identical, forced rebuild, reran: 5/5 green. Both mutation directions are recorded because a single direction would have left half the claim (the false-positive half, or the false-negative half) unarmed.
