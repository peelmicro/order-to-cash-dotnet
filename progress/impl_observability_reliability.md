...
[message updated by backlog id 82, re-run 2026-09-13 — same mutation (o.IncludeScopes = false), forced --no-incremental rebuild. The nameless line above is SUPERSEDED by:]
...
[message updated by backlog id 82, re-run 2026-09-13 — same mutation (ActivityTrackingOptions.None in OrdersHost.cs), forced --no-incremental rebuild. The three nameless lines above are SUPERSEDED by:]
Assert.All() Failure: 3 out of 3 items in the collection did not pass.
[0]: Item:  null
     Error: one of the 3 log records sharing this correlationId carries an empty TraceId scope entry; the values observed were [, , ]. (backlog id 82 - the bare Assert.False named nothing)
[1]: Item:  null
     Error: one of the 3 log records sharing this correlationId carries an empty TraceId scope entry; the values observed were [, , ]. (backlog id 82 - the bare Assert.False named nothing)
[2]: Item:  null
     Error: one of the 3 log records sharing this correlationId carries an empty TraceId scope entry; the values observed were [, , ]. (backlog id 82 - the bare Assert.False named nothing)
...
| `ProjectorDeadLetterTests › OR1_R16_...` (the widened header assertions) | `KafkaDeadLetterPublisher.cs` (Projector copy): `x-first-failed-at` value → the real `FirstFailedAt` render replaced with the literal `"corrupted-by-review-probe"` (the review's own P7 probe) | `Assert.True() Failure / Expected: True / Actual: False` — `DateTimeOffset.TryParse` on the corrupted literal | **[message updated by backlog id 82, re-run 2026-09-13]** the nameless message quoted immediately above is SUPERSEDED: the assertion was replaced by a self-describing one and this arm was re-run against the SAME mutation, on a forced `--no-incremental` rebuild. New verbatim failure: `the .dlq message's x-first-failed-at header is 'corrupted-by-review-probe', which is not a parseable timestamp.` | `cmp` identical; forced rebuild; confirmed green (1/1) |
