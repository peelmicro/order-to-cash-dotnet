...
| 9 | producer is idempotent | `EnableIdempotence = true` → `false` in `KafkaFactPublisher.BuildProducerConfig` | `KafkaFactPublisherConfigTests` › `OI7_Producer_...` | `Assert.True() Failure` — `Expected: True`, `Actual: False` | **[message updated by backlog id 82, re-run 2026-09-13]** the nameless message quoted immediately above is SUPERSEDED: the assertion was replaced by a self-describing one and this arm was re-run against the SAME mutation, on a forced `--no-incremental` rebuild. New verbatim failure: `the fact producer is built with EnableIdempotence = False. Without it an internal librdkafka retry can reorder or duplicate a partition's records (OI7).` |
...
## Retroactive ported-idiom ledger (backlog id 52, added 2026-09-18, audit-only)

> This feature shipped in Phase 8, before the ported-idiom ledger convention became binding at the Phase 8 gate. This row retroactively covers this feature's outbox relay payload read-and-republish boundary; nothing below changes shipped code. See `progress/impl_retroactive_ported_idiom_ledger_decode_boundaries.md` for the full audit — this was the one boundary of the four walked there that exceeded the audit's own 10-minute-per-boundary budget, recorded per the entry's own acceptance bullet 5.

| # | Boundary | #7 relied on | In #8 that property is supplied by |
|---|---|---|---|
| RL4 | Outbox relay payload read-and-republish — `OutboxRelay.BuildPublishableFact` / `OutboxEnvelopeMapper.ToWireBytes` | `outboxRowToEnvelope` (`order-to-cash-nestjs/apps/orders/src/infrastructure/outbox/outbox-envelope-mapper.ts:12-27`): checks all seven envelope fields are present on the claimed row and throws a plain `Error` if any is missing; the row's `payload` column is a native JSON-typed column already parsed by the driver, so no JSON-parse failure mode exists on this read. Its `.map()` call sits OUTSIDE the relay's `try`/`catch` around `this.publisher.publish(...)` (`outbox-relay.ts:143-166`), so a thrown `Error` here is **not** caught by OI8's graceful "leave unstamped, retry unchanged" path — it escapes the transaction uncaught | Partial parity, not exact. `OutboxEnvelopeMapper.ToWireBytes` (`src/Orders/Infrastructure/Outbox/OutboxEnvelopeMapper.cs:26-44`) calls `JsonDocument.Parse(row.Payload)` — because MS-SQL's `payload` column is raw bytes, not a native JSON type, this IS a real decode step #7 never had, and a corrupted row throws `JsonException` here. `BuildPublishableFact` (`OutboxRelay.cs:124/197`) is called in `claimed.Select(...)`, likewise OUTSIDE the `try`/`catch` around `publisher.PublishAsync` (lines 131-134) — matching #7's shape, the exception escapes `RunOnceAsync` uncaught, rolling the transaction back via `await using var transaction`'s disposal. One level up, `OutboxRelayBackgroundService.cs:38-46` catches any non-`OperationCanceledException` from `RunOnceAsync` and logs-and-retries-next-tick — so the *process* does not crash, on both sides. But because the row is rolled back (never stamped `published_at`), and `READPAST` only skips rows still row-locked, the SAME corrupted row would be re-claimed and re-thrown on every subsequent poll — an unbounded retry loop with no poison-row circuit-breaker, on both #7 and #8 equally (checked in #7's `outbox-relay.ts` the same way: its own uncaught `Error` rolls back `this.db.transaction(...)` and the caller (`outbox-relay.service.ts`) is not shown here to special-case it either). **No #7-vs-#8 asymmetry found** — this is inherited, shared debt, not something #8 lost in translation, so "None owed" per `CLAUDE.md`'s ledger rule. Recorded as a finding rather than pursued further because establishing this required reading #7's `outbox-relay.service.ts` caller (not done — see below) to fully confirm the outer retry-loop claim, which pushed this boundary past the audit's 10-minute budget |

**Gaps found:** none confirmed (no #8-only weakness), but **the audit's own stopping rule fired**: `outbox-relay.service.ts` (the caller of `runOnce()`, and the piece that would confirm or refute the "unbounded retry on a poison row" claim on #7's side) was not read. This is listed as a backlog-entry candidate below rather than pushed through.
