...
> This feature shipped in Phase 8, before the ported-idiom ledger convention became binding at the Phase 8 gate (`fulfillment_stock`, id 17, is on record as "the first to carry one" — `specs/fulfillment_stock/design.md:21`). These two rows retroactively cover this feature's two RPC decode boundaries; nothing below changes shipped code. See `progress/impl_retroactive_ported_idiom_ledger_decode_boundaries.md` for the full audit.

| # | Boundary | #7 relied on | In #8 that property is supplied by |
|---|---|---|---|
| RL1 | NATS RPC client reply decode — `NatsStockAvailabilityChecker.CheckAsync` | A `try`/`catch` around `this.replyCodec.decode(reply.data)`, catching a malformed-JSON reply body and rethrowing `StockCheckTransportError` (`order-to-cash-nestjs/apps/orders/src/infrastructure/messaging/nats-stock-availability.adapter.ts:88-96`) | The identical shape, hand-built: a `try`/`catch (JsonException)` around `RpcJson.Deserialize<StockCheckReplyPayload>` at `src/Orders/Infrastructure/Messaging/NatsStockAvailabilityChecker.cs:94-104`, already citing #7's file inline in its own comment (added as Advisory A1's round-2 fix under feature 46 — the malformed-JSON catch feature 46's review found unported the first time). No gap: the property is present and was verified once already; this row only makes it a formal ledger entry |
| RL2 | RPC responder request decode — `OrdersCreateResponder.HandleOrdersCreateAsync` | `@nestjs/microservices`'s NATS transport JSON-decodes the inbound `orders.create` payload BEFORE it reaches the controller — `payload: unknown` in `order-to-cash-nestjs/apps/orders/src/presentation/orders-create.controller.ts:55` is already an object; a malformed body never reaches application code at all, handled entirely by the framework | Feature `fulfillment_stock`'s ledger row L9 records that #8 chose the bare-JSON wire (`INatsConnection.SubscribeAsync<byte[]>`) deliberately, in place of a framework transport — so unlike #7, #8 must hand-build the catch #7 got for free. It does: `RpcJson.Deserialize<OrdersCreateRequestPayload>(message.Data)` at `src/Orders/Presentation/OrdersCreateResponder.cs:131` sits inside the blanket `catch (Exception ex) when (ex is not OperationCanceledException)` at line 140, which maps any decode failure through `OrdersCreateErrorMapper.Map` into a reply rather than crashing the subscribe loop. No gap, but distinct from RL1: here #8 does strictly more work than #7 because it deliberately gave up the framework's decode step |

**Gaps found:** none. Both rows are parity or #8-does-more; no backlog entry filed for either.
