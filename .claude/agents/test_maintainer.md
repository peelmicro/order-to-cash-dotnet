...
1. **Never edit a non-test file.** Only files under `tests/` (`*Tests.cs`) and
   `apps/web/**/*.test.tsx`. If a test cannot pass without a source change,
   **stop and report it** — do not touch the source.
2. **xUnit for the backend, Vitest for `apps/web`.** No Jest, anywhere.
3. **Never weaken a test to make it pass.** Deleting an assertion, loosening it
   to `Assert.NotNull`, or `[Fact(Skip = …)]` on a case is a regression disguised
